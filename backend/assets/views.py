...
from rest_framework.exceptions import APIException, MethodNotAllowed, PermissionDenied, ValidationError as DRFValidationError
...
ASSET_EXPORT_HEADERS = {
    "zh-CN": (
        ...
        "数据中心", "机房", "机柜", "起始 U", "结束 U", "业务 IP", "管理 IP", "带外 IP", "采购日期", "供应商", "采购单号", "采购金额", "折旧方法", "折旧起算日", "折旧年限", "残值率", "资产原值", "预计残值", "月折旧额", "累计折旧", "当前净值", "折旧状态", "维保厂商", "维保合同号", "维保开始日", "维保到期日", "维保备注", "备注", "标签",
    ),
    "en-US": (
        "Asset code", "Asset name", "Model source", "Standard model name", "Model number", "Historical model text", "Manufacturer", "Device type", "Actual warranty (months)", "Expected service life (months)", "Serial number", "Purpose", "Status", "User", "Assigned person employee no.", "Assigned person department", "Assigned person organization", "Assigned person contact",
        "Data center", "Server room", "Rack", "Start U", "End U", "Business IP", "Management IP", "OOB IP", "Purchase date", "Supplier", "Purchase order no.", "Purchase amount", "Depreciation method", "Depreciation start date", "Depreciation years", "Residual rate", "Original asset value", "Estimated residual value", "Monthly depreciation", "Accumulated depreciation", "Current net book value", "Depreciation status", "Maintenance provider", "Maintenance contract no.", "Maintenance start date", "Maintenance expiry date", "Maintenance notes", "Notes", "Tags",
    ),
}
ASSET_EXPORT_SHEET_TITLES = {"zh-CN": "资产台账", "en-US": "Asset ledger"}
ASSET_EXPORT_FILENAMES = {"zh-CN": "资产台账", "en-US": "asset-ledger"}
ASSET_EXPORT_STATUS_LABELS = {
    "zh-CN": ASSET_STATUS_LABELS,
    "en-US": {
        "in_stock": "In stock",
        "in_use": "In use",
        "idle": "Idle",
...
ASSET_EXPORT_DEPRECIATION_METHOD_LABELS = {
    "zh-CN": {"straight_line": "直线法"},
    "en-US": {"straight_line": "Straight-line"},
}
ASSET_EXPORT_DEPRECIATION_STATUS_LABELS = {
    "zh-CN": {
        "unconfigured": "未配置",
        "not_started": "尚未开始",
        "depreciating": "折旧中",
...
ASSET_EXPORT_MODEL_SOURCE_LABELS = {
    "zh-CN": {"standard": "标准型号", "legacy": "历史录入"},
    "en-US": {"standard": "Standard model", "legacy": "Legacy entry"},
}
ASSET_EXPORT_BOOLEAN_LABELS = {
    "zh-CN": {True: "是", False: "否"},
    "en-US": {True: "Yes", False: "No"},
}
AUDIT_EXPORT_HEADERS = {
    "zh-CN": ("时间", "操作人", "资源类型", "动作", "资源 ID", "变更前", "变更后", "附加信息"),
    "en-US": ("Time", "Actor", "Resource type", "Action", "Resource ID", "Before", "After", "Additional information"),
}
AUDIT_EXPORT_SHEET_TITLES = {"zh-CN": "操作日志", "en-US": "Audit logs"}
AUDIT_EXPORT_FILENAMES = {"zh-CN": "操作日志", "en-US": "audit-logs"}
AUDIT_EXPORT_DELETED_ACTOR_LABELS = {"zh-CN": "已删除账号", "en-US": "Deleted account"}
AUDIT_EXPORT_LIMIT_LABELS = {"zh-CN": "操作日志导出结果", "en-US": "Audit log export result"}
...
def _normalize_export_locale(value):
    normalized = str(value or "").strip().lower().replace("_", "-")
    return "en-US" if normalized in {"en", "en-us"} else "zh-CN"


def _export_locale(request):
...
def _asset_export_locale(request):
    return _export_locale(request)
...
class PersonViewSet(AuditedModelViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.action == "list" and _compact_requested(self.request):
            queryset = Person.objects.select_related("department", "account").only(
                "id",
                "name",
                "employee_no",
                "department_id",
...
class AssetViewSet(AuditedModelViewSetMixin, viewsets.ModelViewSet):
    ...
    def _apply_bulk_edit(self, request, instance, changes, batch_operation_id):
        """Apply the explicitly selected asset fields inside one asset transaction."""
        before = asset_audit_snapshot(instance.pk)
        write_data = {}
        changed_fields = []

        model_change = changes.get("model", {})
        if model_change.get("enabled"):
            changed_fields.append("model")
            if model_change["mode"] == "set":
                write_data["asset_model"] = model_change["asset_model"]
            else:
                write_data.update({
                    "asset_model": None,
                    "device_type": model_change["device_type"],
                    "manufacturer": model_change.get("manufacturer"),
                    "model_text": model_change.get("model_text", ""),
                })

        purpose_change = changes.get("purpose", {})
        if purpose_change.get("enabled"):
            changed_fields.append("purpose")
            write_data["purpose"] = "" if purpose_change["action"] == "clear" else purpose_change.get("value", "")

        tag_change = changes.get("tags", {})
        if tag_change.get("enabled"):
            changed_fields.append("tags")
            requested_tag_ids = tag_change.get("values", [])
            requested_tags = list(Tag.objects.filter(pk__in=requested_tag_ids))
            if len(requested_tags) != len(requested_tag_ids):
                raise DRFValidationError({"tags": "存在不存在的标签"})
            inactive_new = [tag.name for tag in requested_tags if not tag.is_active]
            existing_tag_ids = set(AssetTag.objects.filter(asset_id=instance.pk).values_list("tag_id", flat=True))
            if inactive_new and any(tag_id not in existing_tag_ids for tag_id in requested_tag_ids):
                raise DRFValidationError({"tags": "停用的标签不能用于资产"})
            requested_by_id = {tag.pk: tag for tag in requested_tags}
            if tag_change["mode"] == "add":
                final_ids = list(dict.fromkeys([*existing_tag_ids, *requested_by_id]))
            elif tag_change["mode"] == "remove":
                remove_ids = set(requested_by_id)
                final_ids = [tag_id for tag_id in existing_tag_ids if tag_id not in remove_ids]
            else:
                final_ids = list(dict.fromkeys(requested_by_id))
            write_data["tags"] = final_ids
        ...
        location_change = changes.get("location", {})
        if location_change.get("enabled"):
            ...
            if location_change["action"] == "clear":
                ...
                room = ServerRoom.objects.select_related("data_center").filter(
                    pk=location_change["server_room"],
                    data_center_id=location_change["data_center"],
                ).first()
                rack = Rack.objects.select_related("room__data_center").filter(
                    pk=location_change["rack"],
                    room_id=location_change["server_room"],
                ).first()
                if not data_center:
                    raise DRFValidationError({"location.data_center": "数据中心不存在"})
                if not room:
        ...
        after = asset_audit_snapshot(instance.pk)
        write_audit_log(
            request,
            action="bulk_update",
            resource_type="asset",
    ...
    @extend_schema(
        request=AssetBulkEditSerializer,
        responses=AssetBulkEditResponseSerializer,
        description="批量编辑资产型号、位置、标签和用途；每条资产独立事务处理。",
    )
    @action(detail=False, methods=["post"], url_path="bulk-edit")
    def bulk_edit(self, request):
        request_serializer = AssetBulkEditSerializer(data=request.data)
        request_serializer.is_valid(raise_exception=True)
        validated = request_serializer.validated_data
        batch_operation_id = uuid4()
        results = []
        succeeded = 0
        for asset_id in validated["ids"]:
            asset_no = f"ID {asset_id}"
            try:
                with transaction.atomic():
            ...
            except (DRFValidationError, APIException) as exc:
                results.append({
                    "id": asset_id,
                    "asset_no": asset_no,
                    "success": False,
                    "code": _batch_error_code(exc, "BULK_EDIT_REJECTED"),
                    "reason": _batch_error_message(exc, "资产当前不能执行批量编辑"),
                })
        ...
        response_data = {
            "requested": len(validated["ids"]),
            "succeeded": succeeded,
            "failed": len(validated["ids"]) - succeeded,
            "results": results,
        }
        return Response(AssetBulkEditResponseSerializer(response_data).data)


class AttachmentViewSet(viewsets.ModelViewSet):
    queryset = Attachment.objects.select_related("asset", "repair__fault", "uploaded_by").order_by("-uploaded_at", "-id")
    ...
    @staticmethod
    def _parse_query_id(value, field):
        if value in (None, ""):
            return None
        try:
            parsed = int(value)
        except (TypeError, ValueError) as exc:
            raise DRFValidationError({field: "必须是正整数"}) from exc
        if parsed <= 0:
            raise DRFValidationError({field: "必须是正整数"})
        return parsed
    ...
    def _scope_for_instance(self, instance):
        return "faults" if instance.repair_id else "assets"

    def get_object(self):
        instance = super().get_object()
        self._require(self.request, f"{self._scope_for_instance(instance)}.manage" if self.request.method not in {"GET", "HEAD", "OPTIONS"} else f"{self._scope_for_instance(instance)}.view")
        return instance

    def get_queryset(self):
        queryset = super().get_queryset()
        asset_id = self._parse_query_id(self.request.query_params.get("asset"), "asset")
        repair_id = self._parse_query_id(self.request.query_params.get("repair"), "repair")
        if self.action == "list" and asset_id is None and repair_id is None:
            raise DRFValidationError("请指定 asset 或 repair 查询附件")
        if self.action == "list" and asset_id is not None and repair_id is not None:
            raise DRFValidationError("请只指定 asset 或 repair 查询附件")
        if asset_id is not None:
            # Asset detail owns direct asset materials. Repair attachments are
            # exposed through the repair scope so a user with only
            # ``assets.view`` cannot enumerate maintenance documents.
            queryset = queryset.filter(asset_id=asset_id, repair_id__isnull=True)
            if self.action == "list":
                self._require(self.request, "assets.view")
        if repair_id is not None:
            queryset = queryset.filter(repair_id=repair_id)
            if self.action == "list":
                self._require(self.request, "faults.view")
        return queryset

    def create(self, request, *args, **kwargs):
        if request.data.get("repair") not in (None, ""):
            self._require(request, "faults.manage")
        else:
            self._require(request, "assets.manage")
        return super().create(request, *args, **kwargs)

    @action(detail=False, methods=["get"], url_path="categories")
    def categories(self, request):
        scope = request.query_params.get("scope", "asset").strip().lower()
        if scope not in {"asset", "repair"}:
            raise DRFValidationError({"scope": "scope 必须是 asset 或 repair"})
        self._require(request, "faults.view" if scope == "repair" else "assets.view")
        repair_categories = Attachment.REPAIR_CATEGORIES
        values = []
        for value, label in Attachment.CATEGORY_CHOICES:
            if scope == "repair" and value not in repair_categories:
                continue
            if scope == "asset" and value == Attachment.MAINTENANCE_REPORT:
                continue
            values.append({"value": value, "label": label, "scopes": [scope]})
    ...
    def download(self, request, pk=None):
        ...
        if not instance.file or not instance.file.storage.exists(instance.file.name):
            raise DRFValidationError("附件文件不存在")
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            write_audit_log(
                ...
                extra={"asset_id": instance.asset_id, "repair_id": instance.repair_id, "category": instance.category},
            )
        except Exception:
            if instance is not None and instance.file:
                cleanup_attachment_file(instance.file.storage, instance.file.name)
            raise

    @transaction.atomic
    def perform_update(self, serializer):
        before = AttachmentSerializer(serializer.instance).data
        old_name = serializer.instance.file.name
        instance = None
        try:
            instance = serializer.save()
            write_audit_log(
            ...
            if instance is not None and instance.file:
                cleanup_attachment_file(
                    instance.file.storage,
                    instance.file.name,
                    protected_name=old_name,
                )
            raise
...
class RackViewSet(AuditedModelViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.action == "list" and _compact_requested(self.request):
            queryset = Rack.objects.select_related("room__data_center").only(
                "id",
                "room_id",
                "room__id",
                "room__name",
                "room__data_center_id",
                "room__data_center__id",
                "room__data_center__name",
                "code",
                "name",
                "total_u",
                "is_active",
...
class ServerRoomViewSet(AuditedModelViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.action == "list" and _compact_requested(self.request):
            queryset = ServerRoom.objects.select_related("data_center").only(
                "id",
                "data_center_id",
                "data_center__id",
                "data_center__name",
...
class DataCenterViewSet(AuditedModelViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.action == "list" and _compact_requested(self.request):
            queryset = DataCenter.objects.only("id", "name", "address", "is_active")
        else:
            queryset = super().get_queryset()
...
class DictionaryViewSet(AuditedModelViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.action == "list" and _compact_requested(self.request):
            queryset = self.queryset.model._default_manager.all().only(
                "id", "name", "is_active"
            )
        else:
...
class DepartmentViewSet(AuditedModelViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.action == "list" and _compact_requested(self.request):
            queryset = Department.objects.select_related("parent").only(
                "id",
                "name",
                "code",
                "parent_id",
...
class DeviceTypeViewSet(DictionaryViewSet):
    ...
    def get_queryset(self):
        if self.action == "list" and _compact_requested(self.request):
            queryset = DeviceType.objects.select_related("default_fieldset").only(
                "id",
                "name",
                "color",
                "is_active",
                "default_fieldset_id",
                "default_fieldset__id",
                "default_fieldset__name",
            )
        else:
            queryset = self.queryset.annotate(
                assets_count=Count("standalone_assets", distinct=True) + Count("asset_models__assets", distinct=True),
    ...
    def get_queryset(self):
        ...
        if self.action == "list" and _compact_requested(self.request):
            queryset = queryset.only(
                ...
                "device_type__id",
                "device_type__name",
                "device_type__default_fieldset_id",
                "device_type__default_fieldset__id",
                "device_type__default_fieldset__name",
                "fieldset_id",
                "fieldset__id",
                "fieldset__name",
                "model_number",
...
class CustomFieldSetViewSet(AuditedModelViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.action == "list" and _compact_requested(self.request):
            queryset = CustomFieldSet.objects.only("id", "name", "description", "is_active")
        else:
            queryset = super().get_queryset()
...
class TagViewSet(AuditedModelViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.action == "list" and _compact_requested(self.request):
            queryset = Tag.objects.only("id", "name", "is_active")
        else:
            queryset = super().get_queryset()
        if self.action not in {"retrieve", "update", "partial_update", "destroy"}:
            active = self.request.query_params.get("is_active", "true").strip().lower()
...
class SparePartViewSet(AuditedModelViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.action == "list" and _compact_requested(self.request):
            queryset = SparePart.objects.select_related("manufacturer", "category").only(
                "id",
                "code",
                "name",
                "category_id",
...
class SpareStockViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        queryset = super().get_queryset()
        if _compact_requested(self.request):
            queryset = queryset.select_related(None).select_related(
...
def inventory_inspectors(request):
    ...
    if _compact_requested(request):
        ...
        return paginator.get_paginated_response(InventoryInspectorSerializer(rows, many=True).data)
    rows = [
        {"id": user.id, "username": user.username, "display_name": user.get_full_name() or user.username}
        for user in users
    ]
...
class AuditLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def export(self, request):
        """Export the current audit query without exposing sensitive payload keys."""
        errors = _date_filter_errors(request, ("start", "end"))
        if errors:
            return Response(errors, status=400)

        export_locale = _export_locale(request)
        queryset = self.filter_queryset(self.get_queryset()).order_by("-created_at", "-id")
        limit_response = _export_limit_response(queryset, AUDIT_EXPORT_LIMIT_LABELS[export_locale])
        if limit_response:
            return limit_response

        book = Workbook()
        sheet = book.active
        sheet.title = AUDIT_EXPORT_SHEET_TITLES[export_locale]
        _append_excel_row(sheet, AUDIT_EXPORT_HEADERS[export_locale])
        for log in queryset:
            payload = json_value(log.payload or {})
            if not isinstance(payload, dict):
                payload = {}

            def payload_text(key):
            ...
            actor = AUDIT_EXPORT_DELETED_ACTOR_LABELS[export_locale]
            if log.actor_id:
                actor = log.actor.get_full_name() or log.actor.username
            _append_excel_row(sheet, [
                system_localtime(log.created_at).replace(tzinfo=None),
                actor,
                log.resource_type,
                log.action,
                log.resource_id,
        ...
        _style_export_sheet(sheet, max_width=48)
        return _xlsx_response(book, f"{AUDIT_EXPORT_FILENAMES[export_locale]}_{_export_timestamp()}.xlsx")
...
TWO_FACTOR_PENDING_USER_KEY = "infrix.pending_2fa_user_id"
TWO_FACTOR_PENDING_SOURCE_KEY = "infrix.pending_2fa_auth_source"
TWO_FACTOR_PENDING_FAILURES_KEY = "infrix.pending_2fa_failures"


def _clear_pending_two_factor(request):
    for key in (
        TWO_FACTOR_PENDING_USER_KEY,
        TWO_FACTOR_PENDING_SOURCE_KEY,
        TWO_FACTOR_PENDING_FAILURES_KEY,
    ):
...
def _locked_security_profile(user):
    """Return a security profile while tolerating pre-signal user rows."""

    profile = _security_profile(user)
    return UserSecurityProfile.objects.select_for_update().get(pk=profile.pk)
...
def _auth_response(user):
    ...
    return {
        ...
        "two_factor_enabled": security_profile.two_factor_enabled,
...
def auth_login(request):
    ...
    if profile.two_factor_enabled:
        request.session[TWO_FACTOR_PENDING_USER_KEY] = user.pk
        request.session[TWO_FACTOR_PENDING_SOURCE_KEY] = authentication.auth_source
        request.session[TWO_FACTOR_PENDING_FAILURES_KEY] = 0
...
def _security_change_requires_session(request):
    """Do not let a bearer token change the account security boundary."""

    if request.auth is not None:
        raise PermissionDenied("请使用浏览器登录会话管理账号安全设置")


def _two_factor_status_payload(profile):
    return {
        "enabled": bool(profile.two_factor_enabled),
        "configured": bool(profile.two_factor_secret_encrypted),
    }
...
def auth_2fa_status(request):
    return Response(_two_factor_status_payload(_security_profile(request.user)))


@extend_schema(request=OpenApiTypes.NONE, responses=OpenApiTypes.OBJECT)
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def auth_2fa_setup(request):
    ...
    with transaction.atomic():
        ...
        if profile.two_factor_enabled:
            return Response(
                {"detail": "双重验证已经启用", "code": "two_factor_already_enabled"},
                status=400,
            )
        secret = generate_totp_secret()
        try:
            encrypted_secret = encrypt_secret(secret, field_name="two-factor secret")
        except ConfigurationSecretError:
            return Response(
                {"detail": "系统尚未配置安全加密密钥", "code": "two_factor_unavailable"},
                status=503,
            )
        profile.two_factor_secret_encrypted = encrypted_secret
        profile.two_factor_confirmed_at = None
        profile.two_factor_last_used_step = None
    ...
    return Response({
        **_two_factor_status_payload(profile),
        "secret": secret,
        "provisioning_uri": provisioning_uri(username=request.user.username, secret=secret),
    })
...
def auth_2fa_confirm(request):
    ...
    with transaction.atomic():
        profile = _locked_security_profile(request.user)
        if profile.two_factor_enabled:
            return Response(_two_factor_status_payload(profile))
        if not profile.two_factor_secret_encrypted:
            return Response(
                {"detail": "请先开始双重验证设置", "code": "two_factor_not_configured"},
                status=400,
            )
        try:
            secret = _two_factor_secret(profile)
        except ConfigurationSecretError:
            return Response(
                {"detail": "双重验证配置不可用", "code": "two_factor_unavailable"},
                status=503,
            )
        ...
        profile.two_factor_confirmed_at = timezone.now()
        profile.two_factor_last_used_step = None
        profile.save(update_fields=[
            "two_factor_enabled",
            "two_factor_confirmed_at",
...
def auth_2fa_disable(request):
    ...
    with transaction.atomic():
        ...
        if not profile.two_factor_secret_encrypted:
            return Response(
                {"detail": "双重验证配置不可用", "code": "two_factor_not_configured"},
                status=400,
            )
        try:
            secret = _two_factor_secret(profile)
...
def auth_2fa_verify(request):
    ...
    if not pending_user_id:
        return Response(
            {"detail": "双重验证会话已过期，请重新登录", "code": "two_factor_session_expired"},
            status=400,
        )
    try:
    ...
    with transaction.atomic():
        ...
        if not profile.two_factor_enabled or not profile.two_factor_secret_encrypted:
            _clear_pending_two_factor(request)
            return Response(
                {"detail": "双重验证未配置", "code": "two_factor_not_configured"},
                status=400,
            )
        try:
            secret = _two_factor_secret(profile)
        ...
        write_audit_log(
            ...
            extra=_auth_audit_extra(
                request,
                user.username,
                auth_source=source or AUTH_SOURCE_LOCAL,
                two_factor=True,
            ),
        )
    return Response(_auth_response(user))
...
@extend_schema(
    ...
    responses=ApiTokenSerializer(many=True),
    methods=["GET"],
)
@api_view(["GET", "POST"])
@permission_classes([IsAuthenticated])
def auth_api_tokens(request):
    if request.method == "GET":
...
@api_view(["DELETE"])
@permission_classes([IsAuthenticated])
def auth_api_token_revoke(request, token_id):
    _security_change_requires_session(request)
...
@extend_schema(
    request=LdapConfigurationUpdateSerializer,
    responses=OpenApiTypes.OBJECT,
    methods=["PATCH"],
)
@extend_schema(
...
def _backup_error_response(error):
    payload = {"detail": error.message, "code": error.code}
...
@extend_schema(
    parameters=[
        ...
        OpenApiParameter(name="locale", type=OpenApiTypes.STR, required=False, enum=["zh-CN", "en-US"], description="导出表头和系统标签使用的语言。"),
    ],
    responses=OpenApiTypes.BINARY,
...
def asset_export(request):
    export_locale = _asset_export_locale(request)
    ...
    headers = list(ASSET_EXPORT_HEADERS[export_locale])
    headers.extend([field.name or field.key for field in custom_fields])
    status_labels = ASSET_EXPORT_STATUS_LABELS[export_locale]
    boolean_labels = ASSET_EXPORT_BOOLEAN_LABELS[export_locale]
    model_source_labels = ASSET_EXPORT_MODEL_SOURCE_LABELS[export_locale]
    depreciation_method_labels = ASSET_EXPORT_DEPRECIATION_METHOD_LABELS[export_locale]
    depreciation_status_labels = ASSET_EXPORT_DEPRECIATION_STATUS_LABELS[export_locale]
    book = Workbook()
    sheet = book.active
    sheet.title = ASSET_EXPORT_SHEET_TITLES[export_locale]
    _append_excel_row(sheet, headers)
    for asset in assets:
        ...
        for item in asset.custom_values.all():
            ...
            if field.field_type in {"text", "textarea"}:
                ...
                value = "" if item.boolean_value is None else boolean_labels[item.boolean_value]
            elif field.field_type == "select":
        ...
        row_values = [
            asset.asset_no, asset.name, model_source_labels["standard"] if asset.asset_model_id else model_source_labels["legacy"],
            ...
            depreciation_status_labels.get(depreciation["status"], depreciation["status"]),
    ...
    return _xlsx_response(book, f"{ASSET_EXPORT_FILENAMES[export_locale]}_{_export_timestamp()}.xlsx")
