...
class AssetModel(Timestamped):
    ...
    model_number_key = models.GeneratedField(
        expression=NullIf("model_number", Value("")),
        output_field=models.CharField(max_length=160, null=True),
        db_persist=True,
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                fields=["manufacturer", "model_number_key"],
                name="uniq_asset_model_number_key_per_manufacturer",
            ),
        ]
...
class SpareStockTransaction(Timestamped):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["part", "-created_at", "-id"], name="assets_stock_tx_part_idx"),
            models.Index(fields=["-created_at", "-id"], name="assets_stock_tx_recent_idx"),
        ]
...
class SoftwareLicense(Timestamped):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["expiry_date", "name", "id"], name="assets_license_expiry_idx"),
        ]
        constraints = [
            models.CheckConstraint(
...
class Asset(Timestamped):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["status", "asset_no", "id"], name="assets_asset_status_no_idx"),
        ]
...
class NotificationDelivery(Timestamped):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["window_date", "status", "-attempted_at", "-id"], name="assets_delivery_filter_idx"),
            models.Index(fields=["-attempted_at", "-id"], name="assets_delivery_recent_idx"),
        ]
...
class AssetCustomValue(Timestamped):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["field", "asset"], name="assets_custom_field_asset_idx"),
        ]
...
class AssetTag(Timestamped):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["tag", "asset"], name="assets_assettag_tag_asset_idx"),
        ]
...
class RackUnitAllocation(Timestamped):
    ...
    class Meta:
        indexes = [
            models.Index(fields=["rack", "start_u", "end_u"], name="assets_rack_u_range_idx"),
        ]
...
class MaintenanceContract(Timestamped):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["expiry_date", "asset"], name="assets_maint_expiry_asset_idx"),
        ]
...
class FaultEvent(Timestamped):
    ...
    class Meta:
        indexes = [
            models.Index(fields=["is_closed", "-occurred_at", "-id"], name="assets_fault_state_time_idx"),
        ]
...
def attachment_upload_to(instance, filename):
    """Store attachments outside the user-provided filename namespace."""
    suffix = Path(str(filename or "")).suffix.lower()
    return f"attachments/{timezone.now():%Y/%m}/{uuid4().hex}{suffix}"


class Attachment(models.Model):
    """A private document or image attached to an asset or repair record."""
    ...
    REPAIR_CATEGORIES = {PHOTO, MAINTENANCE_REPORT, OTHER}
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["asset", "-uploaded_at", "-id"]),
            models.Index(fields=["repair", "-uploaded_at", "-id"]),
        ]

    def clean(self):
        super().clean()
        if self.category == self.MAINTENANCE_REPORT and not self.repair_id:
            raise ValidationError({"repair": "维修报告必须关联维修记录"})
        if self.repair_id and self.repair and self.repair.fault.asset_id != self.asset_id:
            raise ValidationError({"repair": "维修记录不属于所选资产"})
        if self.repair_id and self.category not in self.REPAIR_CATEGORIES:
            raise ValidationError({"category": "维修附件只能使用照片、维修报告或其他分类"})


@receiver(post_delete, sender=Attachment)
def delete_attachment_file(sender, instance, **kwargs):
    """Delete private storage only after a successful database transaction."""
...
class InventoryTask(Timestamped):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["status", "end_at", "id"], name="assets_inv_task_due_idx"),
            models.Index(fields=["data_center", "server_room", "-created_at", "-id"], name="assets_inv_task_scope_idx"),
        ]
...
class InventoryItem(Timestamped):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["task", "status", "resolution_status", "id"], name="assets_inv_item_filter_idx"),
        ]
...
class AuditLog(Timestamped):
    ...
    class Meta:
        indexes = [
            models.Index(fields=["-created_at", "-id"], name="assets_audit_recent_idx"),
            models.Index(fields=["resource_type", "resource_id", "-created_at", "-id"], name="assets_audit_resource_idx"),
        ]
...
class UserSecurityProfile(Timestamped):
    ...
    two_factor_secret_encrypted = models.TextField(blank=True, default="")
    two_factor_confirmed_at = models.DateTimeField(null=True, blank=True)
    two_factor_last_used_step = models.PositiveBigIntegerField(null=True, blank=True)

    def __str__(self):
        return f"{self.user} security profile"


class PersonalAccessToken(Timestamped):
    """A revocable, hash-only token for integrations and API clients."""
    ...
    class Meta:
        ordering = ["-created_at", "-id"]
        indexes = [
            models.Index(fields=["user", "revoked_at", "created_at"], name="assets_pat_user_state_idx"),
        ]
