...
def _alert_counts(*, today, now, setting, include_assets, include_licenses, include_faults, include_inventory, include_spares):
    ...
    if include_assets and setting.notify_maintenance:
        ...
        maintenance_counts = maintenance.aggregate(
            critical=Count("id", filter=Q(expiry_date__lt=today)),
            warning=Count("id", filter=Q(expiry_date__gte=today)),
        )
        counts["critical"] += maintenance_counts["critical"] or 0
        counts["warning"] += maintenance_counts["warning"] or 0

    if include_licenses:
        license_filters = {
            "over_limit": Q(used_count__gt=F("authorized_count")),
        }
        ...
        license_counts = SoftwareLicense.objects.aggregate(
            **{
                key: Count("id", filter=condition)
                for key, condition in license_filters.items()
            }
        )
        counts["critical"] += license_counts["over_limit"] or 0
        if setting.notify_license_expiry:
            counts["critical"] += license_counts["expired"] or 0
            counts["warning"] += license_counts["expiring"] or 0
