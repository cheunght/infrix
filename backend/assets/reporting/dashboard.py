...
def build_dashboard_payload(scope, *, include_faults=True, include_licenses=True):
    ...
    payload = {
        ...
        "racks": {
            ...
            "free_u": max(capacity["total_u"] - capacity["used_u"], 0),
...
def _build_asset_distributions(scope):
    status_counts = Counter(asset.status for asset in scope.asset_rows)
    asset_total = len(scope.asset_rows)
    status_distribution = [
        {
            "status": status,
            "label": label,
...
def _build_expiry_data(scope, *, today=None):
    ...
    count_row = contracts.aggregate(
        expired=Count("id", filter=Q(expiry_date__lt=today)),
        within_30_days=Count(
            "id",
            filter=Q(expiry_date__gte=today, expiry_date__lte=expiry_30),
        ),
        within_60_days=Count(
            "id",
            filter=Q(expiry_date__gte=today, expiry_date__lte=expiry_60),
        ),
        within_90_days=Count(
            "id",
            filter=Q(expiry_date__gte=today, expiry_date__lte=expiry_90),
        ),
    )
    counts = {key: count_row[key] or 0 for key in (
        "expired",
        "within_30_days",
        "within_60_days",
        "within_90_days",
