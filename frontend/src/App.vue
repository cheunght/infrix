...
const { checkAuth, login, verifyTwoFactor, cancelTwoFactor, logout, loadProfile, saveProfile, changePassword, openPasswordModal } = auth;
const overlayAuth = {
  username,
  loadProfile,
  showPasswordModal,
...
const assetsApi = useAssets({
  ...
  goToLedger: (query: Record<string, string> = {}) => navigateToRoute(
    Object.keys(query).length ? { name: "assets", query } : routeForPage("ledger"),
    true,
  ),
  clearRouteQuery,
  updateRouteQuery,
...
const rackViewFilterQueryKeys = ["data_center", "room", "rack", "rack_code", "device_type"] as const;
let rackViewFiltersInitialized = false;
let rackViewAppliedQueryKey = "";
let pendingRackViewQueryKey: string | null = null;

function rackViewQueryKey(query: unknown): string {
  const source = query && typeof query === "object"
    ? query as Record<string, unknown>
    : {};
  return JSON.stringify(rackViewFilterQueryKeys.map((key) => {
    const value = source[key];
    return Array.isArray(value) ? String(value[0] ?? "") : String(value ?? "");
  }));
}

function markRackViewQueryPending(query: unknown) {
  const key = rackViewQueryKey(query);
  rackViewFiltersInitialized = true;
  rackViewAppliedQueryKey = key;
  pendingRackViewQueryKey = key;
}

function settleRackViewQueryPending(targetKey: string) {
  if (pendingRackViewQueryKey !== targetKey) return;
  const currentKey = rackViewQueryKey(route.query);
  pendingRackViewQueryKey = null;
  if (route.name === "racks-view" && currentKey !== targetKey) {
    rackViewFiltersInitialized = false;
    rackViewAppliedQueryKey = "";
    syncRouteState();
  }
}
...
function syncRouteState(): boolean {
  ...
  let rackViewRouteSyncDeferred = false;
  ...
  if (routePage === "racks" && rackSection.value === "view") {
    ...
    const currentQueryKey = rackViewQueryKey(route.query);
    if (pendingRackViewQueryKey && pendingRackViewQueryKey !== currentQueryKey) {
      // A local selection has already updated the refs and is waiting for its
      // router navigation to settle. Do not write an intermediate old query
      // back into the controls or trigger a request for that stale state.
      rackViewRouteSyncDeferred = true;
    } else {
      if (pendingRackViewQueryKey === currentQueryKey) pendingRackViewQueryKey = null;
      if (!rackViewFiltersInitialized || rackViewAppliedQueryKey !== currentQueryKey) {
        selectedDataCenter.value = dataCenterQuery;
        selectedRoom.value = roomQuery;
      ...
      rackViewFiltersInitialized = true;
      rackViewAppliedQueryKey = currentQueryKey;
    }
  ...
  if (rackViewRouteSyncDeferred) return true;
...
function navigateToRoute(location: RouteLocationRaw, reloadIfSame = false) {
  ...
  const targetRackViewKey = target.name === "racks-view" ? rackViewQueryKey(target.query) : null;
  void router.push(location).then(
    () => {
      if (targetRackViewKey) settleRackViewQueryPending(targetRackViewKey);
    },
    () => {
      if (targetRackViewKey) settleRackViewQueryPending(targetRackViewKey);
    },
...
function clearRouteQuery(keys: string[]): boolean {
  ...
  if (changed) {
    const isRackView = routeName === "racks-view" && keys.some((key) => rackViewFilterQueryKeys.includes(key as typeof rackViewFilterQueryKeys[number]));
    if (isRackView) markRackViewQueryPending(nextQuery);
    const targetRackViewKey = isRackView ? rackViewQueryKey(nextQuery) : null;
    void router.replace({ name: routeName, query: nextQuery }).then(
      () => {
        if (targetRackViewKey) settleRackViewQueryPending(targetRackViewKey);
      },
      () => {
...
function updateRouteQuery(updates: Record<string, string | undefined>): boolean {
  ...
  if (changed) {
    const isRackView = routeName === "racks-view" && Object.keys(updates).some((key) => rackViewFilterQueryKeys.includes(key as typeof rackViewFilterQueryKeys[number]));
    if (isRackView) markRackViewQueryPending(nextQuery);
    const targetRackViewKey = isRackView ? rackViewQueryKey(nextQuery) : null;
    void router.replace({ name: routeName, query: nextQuery }).then(
      () => {
...
function facilitySectionQuery(section: RackSection): Record<string, string> {
  ...
  return {
    ...(selectedDataCenter.value !== null ? { data_center: String(selectedDataCenter.value) } : {}),
    ...(selectedRoom.value !== null ? { room: String(selectedRoom.value) } : {}),
...
async function load() {
  ...
  // These pages own their loading, error and empty states. Avoid stacking the
  // route-level mask on top of their table/card mask during navigation.
  const usesLocalPageLoading =
...
const pageContext = {
  ...
  assetFilters, assetListLoading, assetListError, refreshAssetList, setActionMessage, exportingAssets, resetAssetFilters,
  ...
  changeRoomFilter, changeRackFilter, selectedRoom,
  ...
  assetAuditItems, assetAuditLoading, assetAuditError, assetAuditCanView, retryAssetAudit,
  ...
  <div v-else-if="!authenticated" class="login-screen">
    <div class="login-card">
      ...
      <template v-if="twoFactorRequired">
        <div class="two-factor-login-copy">
          <strong>{{ t('auth.twoFactorRequiredTitle') }}</strong>
          <span>{{ t('auth.twoFactorRequiredDescription') }}</span>
        </div>
        <el-form label-position="top" @submit.prevent="verifyTwoFactor">
          <el-form-item :label="t('auth.twoFactorCode')" required>
            <el-input
          ...
          <div v-if="twoFactorError" class="login-error">{{ twoFactorError }}</div>
          <el-button
            class="login-button"
            type="primary"
            native-type="submit"
            ...
            >{{ t('auth.verifyTwoFactor') }}</el-button
          >
          <el-button class="two-factor-back-button" text :disabled="twoFactorVerifying" @click="cancelTwoFactor">
            {{ t('auth.backToLogin') }}
          </el-button>
        </el-form>
      </template>
