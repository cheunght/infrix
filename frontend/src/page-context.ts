...
export interface AssetLedgerContext {
  ...
  refreshAssetList: () => Promise<boolean>;
  setActionMessage: (message: string, type?: ActionMessageType) => void;
...
export interface AssetAttachmentContext {
  request: RequestFn;
...
export interface RackFiltersContext {
  dataCenters: Ref<DataCenter[]>;
  selectedDataCenter: Ref<number | null>;
  changeDataCenter: () => void | Promise<void>;
  selectedRoom: Ref<number | null>;
  changeRoomFilter: () => void | Promise<void>;
  roomOptions: ComputedRef<Array<{ id: number; name: string; data_center_name?: string }>>;
