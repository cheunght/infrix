...
export function useFacilities(deps: FacilitiesDeps) {
  ...
  // Rack View filter ids stay numeric internally.  Route/query values are
  // converted at the App boundary before they reach these refs.
  const selectedDataCenter = ref<number | null>(null);
  ...
  function rackListPath(pageNumber: number) {
    ...
    if (selectedDataCenter.value !== null) params.set("room__data_center", String(selectedDataCenter.value));
    if (selectedRoom.value !== null) params.set("room", String(selectedRoom.value));
    ...
    try {
      if (selectedRoom.value) {
        try {
          ...
          if (!selectedRoomDetail.is_active) selectedRoom.value = null;
        } catch (error) {
          if (!isAbortError(error)) selectedRoom.value = null;
        }
  ...
  function openRackModal(rack?: Rack, room?: ServerRoom) {
    ...
    if (rack) {
      ...
      const currentRoom = room || serverRooms.value.find((item) => String(item.id) === String(selectedRoom.value));
  ...
  const visibleRacks = computed(() =>
    racks.value.filter(
      (rack) =>
        selectedRoom.value === null || String(rack.room) === String(selectedRoom.value),
    ),
  );
  const roomOptions = computed(() =>
    serverRooms.value
      .filter(
        (room) =>
          room.is_active &&
          (!selectedDataCenter.value || String(room.data_center) === String(selectedDataCenter.value)),
      )
      .map((room) => ({ id: room.id, name: room.name, data_center_name: room.data_center_name })),
  );
  const hasRackFilters = computed(() => Boolean(
    selectedDataCenter.value !== null || selectedRoom.value !== null || selectedRack.value || selectedRackDeviceTypeId.value,
  ));
  ...
  function changeDataCenter() {
    if (selectedRoom.value !== null) {
      const room = serverRooms.value.find((item) => String(item.id) === String(selectedRoom.value));
      const roomBelongsToSelectedCenter = Boolean(
        room && (
          selectedDataCenter.value === null
          || String(room.data_center) === String(selectedDataCenter.value)
        ),
      );
      if (!roomBelongsToSelectedCenter) selectedRoom.value = null;
    }
    selectedRack.value = "";
    ...
    if (deps.updateRouteQuery?.({
      data_center: selectedDataCenter.value === null ? undefined : String(selectedDataCenter.value),
      room: selectedRoom.value === null ? undefined : String(selectedRoom.value),
      rack: undefined,
      rack_code: undefined,
