...
export interface AuthDeps {
  ...
  twoFactorCode: Ref<string>;
  twoFactorVerifying: Ref<boolean>;
  twoFactorError: Ref<string>;
...
export function useAuth(deps: AuthDeps) {
  ...
  function errorMessage(error: unknown, fallback: string) {
    ...
    const localizedErrorKeys: Record<string, string> = {
      ...
      two_factor_session_expired: "security.twoFactorSessionExpired",
      two_factor_locked: "security.twoFactorLocked",
    ...
    if (localizedKey && i18n.global.te(localizedKey)) return String(i18n.global.t(localizedKey));
  ...
  function clearSessionIdentity() {
    ...
    deps.twoFactorRequired.value = false;
    deps.twoFactorCode.value = "";
    deps.twoFactorError.value = "";
  ...
  async function finishLogin(user: AuthPayload) {
    applyAuthPayload(user);
    deps.syncRouteState();
    deps.ensureRouteAccess();
    if (!hasCapability(deps.permissions.value, "organization.manage") && deps.settingsSection.value === "organization") {
  ...
  async function login() {
    ...
    try {
      ...
      if (user.two_factor_required) {
        deps.twoFactorRequired.value = true;
        deps.twoFactorCode.value = "";
        deps.password.value = "";
        return;
      }
  ...
  async function verifyTwoFactor() {
    if (deps.twoFactorVerifying.value) return;
    deps.twoFactorVerifying.value = true;
    deps.twoFactorError.value = "";
    deps.bootstrapError.value = false;
    let sessionEstablished = false;
    try {
      ...
      const user = await deps.request<AuthPayload>("/auth/2fa/verify/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ code: deps.twoFactorCode.value.trim() }),
      });
      sessionEstablished = true;
      deps.twoFactorRequired.value = false;
      deps.twoFactorCode.value = "";
      ...
      deps.twoFactorError.value = errorMessage(error, i18n.global.t("auth.twoFactorVerifyFailed"));
    } finally {
      deps.twoFactorVerifying.value = false;
    }
  }
