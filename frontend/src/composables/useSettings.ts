...
const SYSTEM_SETTINGS_FORM_KEYS: Array<keyof SystemSettingsForm> = [
...
export function useSettings(deps: SettingsDeps) {
  ...
  function syncSystemSettingsFormFields(
    value: SystemSettings,
    keys: readonly (keyof SystemSettingsForm)[],
  ): void {
    const form = systemSettingsForm.value as unknown as Record<string, unknown>;
    ...
    for (const key of keys) {
      ...
      form[key] = Array.isArray(nextValue) ? [...nextValue] : nextValue;
    }
  }

  function clearSystemSettingsFormErrors(keys: readonly (keyof SystemSettingsForm)[]): void {
    const clearedKeys = new Set<string>(keys);
    systemSettingsFormErrors.value = Object.fromEntries(
      Object.entries(systemSettingsFormErrors.value).filter(([key]) => !clearedKeys.has(key)),
    );
  }
  ...
  function resetSystemSettingsForm(
    keys: readonly (keyof SystemSettingsForm)[] = SYSTEM_SETTINGS_FORM_KEYS,
  ): void {
    if (systemSettings.value) syncSystemSettingsFormFields(systemSettings.value, keys);
  ...
  async function saveSystemSettings(
    ...
    try {
      ...
      for (const key of submittedKeys) {
        if (key === "smtp_password") {
          if (systemSettingsForm.value.smtp_password) {
            settingsPayload.smtp_password = systemSettingsForm.value.smtp_password;
          }
          continue;
        }
        settingsPayload[key] = systemSettingsForm.value[key];
      }
      const result = await deps.request<SystemSettings>("/system/settings/", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      systemSettings.value = result;
      syncSystemSettingsFormFields(result, submittedKeys);
      applySystemSettingsSnapshot(result);
      setActionMessage(tr("settings.settingsSaved"));
    } catch (error) {
      systemSettingsFormErrors.value = {
        ...systemSettingsFormErrors.value,
        ...extractFieldErrors(error, submittedKeys),
      };
