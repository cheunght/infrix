...
export type MaintenanceTab = "operations" | "backup" | "reset";
...
export function routeForPage(
  ...
  if (page === "settings") {
    ...
    if (section === "maintenance" && options.maintenanceTab && options.maintenanceTab !== "operations") {
      return { name: `settings-${section}`, query: { tab: options.maintenanceTab } };
    }
