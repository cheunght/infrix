...
import { Delete, Download, Edit, Filter, MoreFilled, Operation, Switch, Tickets, TopRight, Upload } from "@element-plus/icons-vue";
...
function mapDictionary(item: Record<string, unknown>): SearchableSelectOption {
  ...
  return { value: String(value.id), label: value.name, secondary: value.code || "", disabled: value.is_active === false, data: value };
}
...
function mapDataCenter(item: Record<string, unknown>): SearchableSelectOption {
  ...
  return { value: String(value.id), label: value.name, secondary: value.address || "", disabled: value.is_active === false, data: value };
}

function mapRoom(item: Record<string, unknown>): SearchableSelectOption {
  const value = item as unknown as ServerRoom;
  return { value: String(value.id), label: value.name, secondary: value.data_center_name || "", disabled: value.is_active === false, data: value };
}
...
const activeSpareFilterCount = computed(() =>
  Number(Boolean(spareCategory.value)) +
  Number(Boolean(spareManufacturer.value)) +
  Number(Boolean(spareListDataCenter.value)) +
  Number(Boolean(spareListRoom.value)),
);
const selectedFilterCategoryOption = computed(() => optionFrom(spareCategories.value as SparePartCategory[], spareFilterDraft.category, mapCategory));
const selectedFilterManufacturerOption = computed(() => optionFrom(manufacturers.value as DictionaryItem[], spareFilterDraft.manufacturer, mapDictionary));
const selectedFilterDataCenterOption = computed(() => optionFrom(dataCenters.value as DataCenter[], spareFilterDraft.dataCenter, mapDataCenter));
const selectedFilterRoomOption = computed(() => optionFrom(spareRooms.value as ServerRoom[], spareFilterDraft.room, mapRoom));
...
const showSpareFilters = ref(false);
const spareFilterDraft = reactive({ category: "", manufacturer: "", dataCenter: "", room: "" });
...
function openSpareFilters() {
  spareFilterDraft.category = spareCategory.value;
  spareFilterDraft.manufacturer = spareManufacturer.value;
  spareFilterDraft.dataCenter = spareListDataCenter.value;
  spareFilterDraft.room = spareListRoom.value;
  showSpareFilters.value = true;
}
function clearSpareFilterDraft() {
  spareFilterDraft.category = "";
  spareFilterDraft.manufacturer = "";
  spareFilterDraft.dataCenter = "";
  spareFilterDraft.room = "";
}
function clearSpareFilterRoomIfNeeded() {
  if (!spareFilterDraft.dataCenter || !spareFilterDraft.room) return;
  const room = (spareRooms.value as ServerRoom[]).find((item) => String(item.id) === spareFilterDraft.room);
  if (room && String(room.data_center) !== spareFilterDraft.dataCenter) spareFilterDraft.room = "";
}
function applySpareFilters() {
  clearSpareFilterRoomIfNeeded();
  spareCategory.value = spareFilterDraft.category;
  spareManufacturer.value = spareFilterDraft.manufacturer;
  spareListDataCenter.value = spareFilterDraft.dataCenter;
  spareListRoom.value = spareFilterDraft.room;
  searchSpareParts();
  showSpareFilters.value = false;
}
...
<template>
  <div class="infrix-page spare-page">
    <PageContainer>
      <template #toolbar>
        <PageToolbar>
          ...
          <template #filters>
            <ToolbarIconButton :icon="Filter" :label="t('asset.moreFilters')" :badge="activeSpareFilterCount" @click="openSpareFilters" />
          </template>
          <template #actions>
            <ToolbarIconButton
      ...
      <PageContent surface class="spare-list-content">
        <ResourceState :error="spareListError" @retry="retrySpareList">
          ...
          <el-table class="spare-table" table-layout="fixed" v-loading="spareListLoading" :data="spareParts" row-key="id" @expand-change="handleExpandChange">
                  ...
                  <div v-else-if="stockLocationLoadedByPart[row.id] && stocksFor(row).length" class="spare-location-list">
                    <div v-for="stock in stocksFor(row)" :key="stock.id" class="spare-location-row">
                      ...
                      <div class="ep-table-actions">
                        ...
                        <el-dropdown v-if="can('spares.manage')" trigger="click" placement="bottom-end">
                          <el-button link :icon="MoreFilled" :aria-label="t('common.more')" :title="t('common.more')" @click.stop />
                          <template #dropdown>
                            <el-dropdown-menu>
                              <el-dropdown-item @click="openLocationOperation(row, 'inbound', stock)">{{ t('spare.inbound') }}</el-dropdown-item>
                              <el-dropdown-item :disabled="stock.quantity <= 0" @click="openLocationOperation(row, 'outbound', stock)">{{ t('spare.outbound') }}</el-dropdown-item>
                              <el-dropdown-item @click="openLocationOperation(row, 'transfer', stock)">{{ t('spare.transfer') }}</el-dropdown-item>
                              <el-dropdown-item @click="openLocationOperation(row, 'adjustment', stock)">{{ t('spare.adjustStock') }}</el-dropdown-item>
                              <el-dropdown-item class="infrix-dropdown-item--danger" :disabled="stock.quantity <= 0" @click="openLocationOperation(row, 'scrap', stock)">{{ t('spare.scrap') }}</el-dropdown-item>
                            </el-dropdown-menu>
                          </template>
                        </el-dropdown>
                      </div>
            ...
            <el-table-column :label="t('common.operation')" fixed="right" width="116">
              <template #default="{ row }">
                <div v-if="can('spares.manage')" class="ep-table-actions">
                  <el-button-group>
                    ...
                    <TableIconButton :icon="Operation" :label="t('spare.adjustStock')" :disabled="deletingSparePartId === row.id" @click="openSpareOperation(row, 'adjustment')" />
    ...
    <el-drawer v-model="showSpareFilters" :title="t('asset.moreFilters')" size="360px" append-to-body>
      <el-form label-position="top">
        <el-form-item :label="t('spare.spareType')">
          <SearchableSelect
            v-model="spareFilterDraft.category"
            :request="context.request"
            endpoint="/spare-part-categories/"
            :map-option="mapCategory"
            :selected-option="selectedFilterCategoryOption"
            :base-query="{ is_active: 'all' }"
            :placeholder="t('spare.spareType')"
            clearable
          />
        </el-form-item>
        <el-form-item :label="t('spare.manufacturer')">
          <SearchableSelect
            v-model="spareFilterDraft.manufacturer"
            :request="context.request"
            endpoint="/manufacturers/"
            :map-option="mapDictionary"
            :selected-option="selectedFilterManufacturerOption"
            :base-query="{ is_active: 'all' }"
            :placeholder="t('spare.manufacturer')"
            clearable
          />
        </el-form-item>
        <el-form-item :label="t('spare.initialDataCenter')">
          <SearchableSelect
            v-model="spareFilterDraft.dataCenter"
            :request="context.request"
            endpoint="/data-centers/"
            :map-option="mapDataCenter"
            :selected-option="selectedFilterDataCenterOption"
            :base-query="{ is_active: 'all' }"
            :placeholder="t('spare.initialDataCenter')"
            clearable
            @select="spareFilterDraft.room = ''"
          />
        </el-form-item>
        <el-form-item :label="t('spare.initialRoom')">
          <SearchableSelect
            v-model="spareFilterDraft.room"
            :request="context.request"
            endpoint="/server-rooms/"
            :map-option="mapRoom"
            :selected-option="selectedFilterRoomOption"
            :base-query="{ data_center: spareFilterDraft.dataCenter, is_active: 'all' }"
            :placeholder="t('spare.initialRoom')"
            clearable
          />
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="page-filter-drawer__footer">
          <el-button @click="clearSpareFilterDraft">{{ t('common.clearFilters') }}</el-button>
          <el-button @click="showSpareFilters = false">{{ t('common.cancel') }}</el-button>
          <el-button type="primary" @click="applySpareFilters">{{ t('asset.applyFilters') }}</el-button>
        </div>
      </template>
    </el-drawer>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="spareSaving" :disabled="spareSaving" @click="handleSaveSparePart">{{ editingSparePart ? t('common.save') : t('common.create') }}</el-button>
      </template>
    </FormDialogShell>
