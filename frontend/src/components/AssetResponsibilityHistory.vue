...
function personName(name: string): string {
  return name || t("asset.unassigned");
}
...
function transitionLabel(item: AssetAssignmentEvent): string {
  return `${personName(item.from_person_name)} → ${personName(item.to_person_name)}`;
}

function transitionDetails(item: AssetAssignmentEvent): string {
  const from = personDetails(
    ...
    >
      ...
      >
        <div v-if="compact" class="asset-history-list">
          ...
          >
            <strong
              class="asset-history-record__title"
              :title="transitionDetails(item)"
              :aria-label="`${actionLabel(item.action)} · ${transitionDetails(item)}`"
            >{{ actionLabel(item.action) }} · {{ transitionLabel(item) }}</strong>
            <span
        ...
        <el-table v-else :data="items" row-key="id" table-layout="fixed">
          ...
          >
            <template #default="{ row }">
              <span
                :title="transitionDetails(row)"
                :aria-label="transitionDetails(row)"
                >{{ transitionLabel(row) }}</span
              >
            </template>
          </el-table-column>
          <el-table-column
