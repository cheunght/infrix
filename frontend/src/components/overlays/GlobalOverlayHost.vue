...
const assetAttachmentContext = computed<AssetAttachmentContext>(() => ({
  request,
  download: props.assetContext.downloadFile,
  confirmAction: props.assetContext.confirmAction,
  canView: props.assetContext.can("assets.view"),
  canManage: props.assetContext.can("assets.manage"),
}));
const repairAttachmentContext = computed<AssetAttachmentContext>(() => ({
  request,
  download: props.assetContext.downloadFile,
  confirmAction: props.assetContext.confirmAction,
  canView: props.assetContext.can("faults.view"),
  canManage: props.assetContext.can("faults.manage"),
}));
...
<template>
  ...
  >
    <div v-if="profileError" class="settings-state settings-state--error profile-state profile-settings-state" role="alert">
    ...
    <template v-else>
      <div class="profile-settings-content">
        <section class="profile-settings-summary" :aria-label="t('auth.accountInfo')">
          <div class="profile-settings-summary__identity">
            <strong>{{ username }}</strong>
            <span aria-hidden="true">·</span>
            <span>{{ localizedRoleName || t('common.notAvailable') }}</span>
            <el-tag :type="userIsActive ? 'success' : 'info'" effect="light">
              {{ userIsActive ? t('status.active') : t('status.inactive') }}
            </el-tag>
          </div>
          <span class="profile-settings-summary__last-login">
            {{ t('auth.lastLogin') }}：{{ formatProfileDateTime(lastLogin) }}
          </span>
        </section>
        ...
        >
          <section class="form-dialog__section profile-settings-section">
            <h3 class="form-dialog__section-title">{{ t('auth.personalInfo') }}</h3>
            <div class="profile-settings-form-grid">
        ...
        <section class="profile-settings-security">
          <div class="profile-settings-section-heading">
            <h3 class="profile-settings-section-heading__title">{{ t('security.accountSecurity') }}</h3>
          </div>
          <AccountSecurityPanel :request="request" :active="showProfileModal" />
        </section>
      </div>
    </template>
    <template #footer>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="licenseSaving" :disabled="licenseSaving" @click="submitLicense">{{ editingLicense ? t('common.save') : t('common.create') }}</el-button>
    </template>
  </FormDialogShell>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="passwordSaving" :disabled="passwordSaving" @click="submitPassword">{{ t('common.save') }}</el-button>
    </template>
  </ActionDialogShell>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="faultSaving" :disabled="faultSaving" @click="submitFault">{{ t('common.create') }}</el-button>
    </template>
  </FormDialogShell>
  ...
  >
    <el-form ref="repairFormRef" class="repair-action-form" :class="{ 'repair-action-form--readonly': repairReadOnly }" :model="repairForm" :rules="repairFormRules" label-position="top" :validate-on-rule-change="false">
      ...
      <AttachmentSection
        v-if="selectedFault?.repair && repairAttachmentContext.canView"
        :asset-id="selectedFault.asset"
        :repair-id="selectedFault.repair.id"
        scope="repair"
        :request="repairAttachmentContext.request"
        :download="repairAttachmentContext.download"
        :confirm-action="repairAttachmentContext.confirmAction"
        :can-view="repairAttachmentContext.canView"
        :can-manage="repairAttachmentContext.canManage && !repairReadOnly"
      />
    </el-form>
    <template #footer>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="repairPartUsageSaving" :disabled="repairPartUsageSaving" @click="submitRepairPartUsage">{{ t('common.save') }}</el-button>
    </template>
  </ActionDialogShell>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="dataCenterSaving" :disabled="dataCenterSaving" @click="submitDataCenter">{{ editingDataCenter ? t('common.save') : t('common.create') }}</el-button>
    </template>
  </FormDialogShell>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="roomSaving" :disabled="roomSaving" @click="submitRoom">{{ editingRoom ? t('common.save') : t('common.create') }}</el-button>
    </template>
  </FormDialogShell>
