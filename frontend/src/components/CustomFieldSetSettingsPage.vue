...
import { ArrowDown, ArrowUp, CircleCheck, CircleClose, Delete, Edit, Filter, Rank } from "@element-plus/icons-vue";
...
const showFieldsetFilters = ref(false);
const fieldsetFilterDraft = reactive({ active: "" });
const canManage = computed(() => c.can("custom_fields.manage"));
const activeFieldsetFilterCount = computed(() => Number(Boolean(active.value)));
...
function openFieldsetFilters() {
  fieldsetFilterDraft.active = active.value;
  showFieldsetFilters.value = true;
}

function clearFieldsetFilters() {
  fieldsetFilterDraft.active = "";
}

function applyFieldsetFilters() {
  active.value = fieldsetFilterDraft.active;
  page.value = 1;
  void loadFieldsets();
  showFieldsetFilters.value = false;
}
...
<template>
  <PageContainer>
    <template #toolbar>
      <PageToolbar>
        ...
        <template #filters><ToolbarIconButton :icon="Filter" :label="t('asset.moreFilters')" :badge="activeFieldsetFilterCount" @click="openFieldsetFilters" /></template>
    ...
    <PageContent surface>
      ...
      <PagedTable v-else v-model:current-page="page" v-model:page-size="pageSize" :total="total" @update:current-page="loadFieldsets" @update:page-size="page = 1; loadFieldsets()">
        <el-table v-loading="loading" :data="rows" table-layout="fixed">
          ...
          <el-table-column v-if="canManage" :label="t('common.operation')" width="116" fixed="right"><template #default="{ row }"><div class="ep-table-actions"><el-button-group><TableIconButton :icon="Edit" :label="t('common.edit')" type="primary" @click="openModal(row)" /><TableIconButton :icon="row.is_active ? CircleClose : CircleCheck" :label="row.is_active ? t('status.inactive') : t('status.active')" @click="toggleStatus(row)" /><TableIconButton :icon="Delete" :label="row.models_count || row.device_types_count ? t('customFieldSet.deleteBlocked') : t('common.delete')" type="danger" :disabled="row.models_count > 0 || row.device_types_count > 0" @click="remove(row)" /></el-button-group></div></template></el-table-column>
        </el-table>
      </PagedTable>
    </PageContent>
  </PageContainer>

  <el-drawer v-model="showFieldsetFilters" :title="t('asset.moreFilters')" size="360px" append-to-body>
    <el-form label-position="top">
      <el-form-item :label="t('common.status')">
        <el-select v-model="fieldsetFilterDraft.active" clearable :placeholder="t('common.status')">
    ...
    <template #footer>
      <div class="page-filter-drawer__footer">
        <el-button @click="clearFieldsetFilters">{{ t('common.clearFilters') }}</el-button>
        <el-button @click="showFieldsetFilters = false">{{ t('common.cancel') }}</el-button>
        <el-button type="primary" @click="applyFieldsetFilters">{{ t('asset.applyFilters') }}</el-button>
      </div>
    </template>
  </el-drawer>
  ...
  <FormDialogShell v-model="showModal" :title="editing ? t('customFieldSet.edit') : t('customFieldSet.create')" :description="t('customFieldSet.help')" size="large" :saving="saving" :close-disabled="saving">
    ...
    <template #footer><el-button :disabled="saving" @click="showModal = false">{{ t('common.cancel') }}</el-button><el-button type="primary" :loading="saving" @click="save">{{ editing ? t('common.save') : t('common.create') }}</el-button></template>
  </FormDialogShell>
</template>
