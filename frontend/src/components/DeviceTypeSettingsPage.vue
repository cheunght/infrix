...
const showDeviceTypeFilters = ref(false);
const deviceTypeFilterDraft = reactive({ active: "" });
const canManage = computed(() => c.can("settings.manage"));
const activeDeviceTypeFilterCount = computed(() => Number(Boolean(active.value)));
...
function openDeviceTypeFilters() {
  deviceTypeFilterDraft.active = active.value;
  showDeviceTypeFilters.value = true;
}

function clearDeviceTypeFilters() {
  deviceTypeFilterDraft.active = "";
}

function applyDeviceTypeFilters() {
  active.value = deviceTypeFilterDraft.active;
  page.value = 1;
  void loadRows();
  showDeviceTypeFilters.value = false;
}
...
<template>
  <PageContainer>
    <template #toolbar>
      <PageToolbar>
        ...
        <template #filters><ToolbarIconButton :icon="Filter" :label="t('asset.moreFilters')" :badge="activeDeviceTypeFilterCount" @click="openDeviceTypeFilters" /></template>
  ...
  <el-drawer v-model="showDeviceTypeFilters" :title="t('asset.moreFilters')" size="360px" append-to-body>
    <el-form label-position="top">
      <el-form-item :label="t('common.status')">
        <el-select v-model="deviceTypeFilterDraft.active" clearable :placeholder="t('common.status')">
    ...
    <template #footer>
      <div class="page-filter-drawer__footer">
        <el-button @click="clearDeviceTypeFilters">{{ t('common.clearFilters') }}</el-button>
        <el-button @click="showDeviceTypeFilters = false">{{ t('common.cancel') }}</el-button>
        <el-button type="primary" @click="applyDeviceTypeFilters">{{ t('asset.applyFilters') }}</el-button>
      </div>
    </template>
  </el-drawer>
