...
const showItemFilters = ref(false);
const itemFilterDraft = reactive({ status: "", resolutionStatus: "" });
...
function mapRack(item: Record<string, unknown>): SearchableSelectOption {
  ...
  return { value: String(rack.id), label: rack.code, secondary: [rack.server_room_name, rack.data_center_name].filter(Boolean).join(" · "), data: rack };
}
...
function mapInspector(item: Record<string, unknown>): SearchableSelectOption {
  ...
  return { value: String(inspector.id), label: inspector.display_name || inspector.username, secondary: inspector.username, data: inspector };
}
...
const activeItemFilterCount = computed(() =>
  Number(Boolean(itemStatus.value)) + Number(Boolean(itemResolutionStatus.value)),
);

function openItemFilters() {
  itemFilterDraft.status = itemStatus.value;
  itemFilterDraft.resolutionStatus = itemResolutionStatus.value;
  showItemFilters.value = true;
}

function clearItemFilterDraft() {
  itemFilterDraft.status = "";
  itemFilterDraft.resolutionStatus = "";
}

function applyItemFilters() {
  itemStatus.value = itemFilterDraft.status as typeof itemStatus.value;
  itemResolutionStatus.value = itemFilterDraft.resolutionStatus as typeof itemResolutionStatus.value;
  itemPage.value = 1;
  void loadItems();
  showItemFilters.value = false;
}
        ...
        <PageToolbar>
          ...
          <template #filters>
            <div class="page-toolbar__filter-group">
              <el-select v-model="taskStatus" class="toolbar-filter--sm" :placeholder="t('inventory.allTaskStatuses')" clearable @change="() => { taskPage = 1; loadTasks(); }">
              ...
              <template #default="{ row }">
                <div class="ep-table-actions">
                  ...
                  <TableIconButton
                    ...
                    @click="openTask(row)"
                  />
                  <TableIconButton v-if="can('inventory.export')" :icon="Download" :label="t('common.export')" :loading="exportingTaskId === row.id" :disabled="exportingTaskId !== null || taskDeletingId !== null" @click="exportTask(row)" />
                  <TableIconButton v-if="can('inventory.manage') && row.can_delete" :icon="Delete" :label="t('common.delete')" type="danger" :loading="taskDeletingId === row.id" :disabled="exportingTaskId !== null || taskDeletingId !== null" @click="deleteTask(row)" />
                  </el-button-group>
                </div>
              </template>
            </el-table-column>
      ...
      <PageContent v-loading="taskDetailLoading">
        ...
        <PageSection :title="t('inventory.inventoryDevices')">
          <PageToolbar>
            ...
            <template #filters>
              <ToolbarIconButton :icon="Filter" :label="t('asset.moreFilters')" :badge="activeItemFilterCount" @click="openItemFilters" />
            </template>
            <template #actions>
          ...
          <el-drawer v-model="showItemFilters" :title="t('asset.moreFilters')" size="360px" append-to-body>
            <el-form label-position="top">
              <el-form-item :label="t('inventory.allInventoryResults')">
                <el-select v-model="itemFilterDraft.status" clearable :placeholder="t('inventory.allInventoryResults')">
                  <el-option v-for="item in itemStatusOptions" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
              </el-form-item>
              <el-form-item :label="t('inventory.allResolutionStatuses')">
                <el-select v-model="itemFilterDraft.resolutionStatus" clearable :placeholder="t('inventory.allResolutionStatuses')">
                  <el-option v-for="item in itemResolutionStatusOptions" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
              </el-form-item>
            </el-form>
            <template #footer>
              <div class="page-filter-drawer__footer">
                <el-button @click="clearItemFilterDraft">{{ t('common.clearFilters') }}</el-button>
                <el-button @click="showItemFilters = false">{{ t('common.cancel') }}</el-button>
                <el-button type="primary" @click="applyItemFilters">{{ t('asset.applyFilters') }}</el-button>
              </div>
            </template>
          </el-drawer>
          ...
          <el-table-column :label="t('common.operation')" fixed="right" width="84">
            <template #default="{ row }">
              <div class="ep-table-actions">
                <el-button-group>
                <TableIconButton
                ...
                />
                <TableIconButton
                  v-if="itemCanEditException(row) && !itemPrimaryAction(row)"
                  :icon="Edit"
