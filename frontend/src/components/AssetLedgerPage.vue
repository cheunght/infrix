...
import type { Asset, AssetBatchAssignmentAction, AssetCustomFilter, AssetStatus, CustomFieldFilterOperator, PersonOption } from "../types";
...
import BulkEditAssetDialog from "./BulkEditAssetDialog.vue";
...
function openBulkEdit() {
  if (!can("assets.manage") || !selectedAssetIds.value.length || selectedAssetIds.value.length > 100) return;
  showBulkEditDialog.value = true;
}

async function handleBulkEditCompleted(response: import("../types").AssetBulkEditResponse) {
  const failedIds = new Set(response.results.filter((item) => !item.success).map((item) => item.id));
  selectedAssetIds.value = [];
  const refreshed = await refreshAssetList();
  if (!refreshed) {
    setActionMessage(t("asset.bulkEditRefreshFailed"), "error");
  }
  selectedAssetIds.value = assets.value.filter((asset) => failedIds.has(asset.id)).map((asset) => asset.id);
  await nextTick();
  assetTableRef.value?.clearSelection();
  for (const asset of assets.value) {
    if (failedIds.has(asset.id)) assetTableRef.value?.toggleRowSelection(asset, true);
  }
}

async function handleBulkEditUnknown() {
  const refreshed = await refreshAssetList();
  setActionMessage(
    t(refreshed ? "asset.bulkEditNetworkUnknown" : "asset.bulkEditNetworkRefreshFailed"),
    "error",
  );
}
...
const selectedDeviceTypeOption = computed<SearchableSelectOption | null>(() => {
  const item = activeDeviceTypes.value.find((entry) => String(entry.id) === advancedFilters.deviceType);
...
const selectedTagOptions = computed<SearchableSelectOption[]>(() => activeTags.value
  .filter((item) => advancedFilters.tag.includes(String(item.id)))
...
const activeFilterCount = computed(() =>
  Number(Boolean(assetFilters.status)) +
  Number(Boolean(assetFilters.deviceType)) +
  Number(assetFilters.tag.length > 0) +
  Number(Boolean(assetFilters.manufacturer)) +
  Number(Boolean(assetFilters.model.trim())) +
  Number(Boolean(assetFilters.dataCenter)) +
  Number(Boolean(assetFilters.warranty)) +
  appliedCustomFilters.value.length,
);
...
const activeFilterChips = computed(() => {
  ...
  if (assetFilters.tag.length) chips.push({ key: "tag", label: `${t("asset.tag")}：${activeTags.value.filter((item) => assetFilters.tag.includes(String(item.id))).map((item) => item.name).join(", ") || assetFilters.tag.join(", ")}` });
...
function openAdvancedFilters() {
  advancedFilters.status = assetFilters.status;
  advancedFilters.deviceType = assetFilters.deviceType;
  advancedFilters.tag = [...assetFilters.tag];
...
async function applyAdvancedFilters() {
  assetFilters.status = advancedFilters.status;
  assetFilters.deviceType = advancedFilters.deviceType;
  assetFilters.tag = [...advancedFilters.tag];
...
<template>
        ...
        <PageToolbar class="asset-ledger-toolbar">
          ...
          <template #actions>
            ...
            <div v-if="can('assets.manage') || can('assets.export')" class="asset-ledger-action-group asset-ledger-action-group--data" role="group" :aria-label="t('asset.dataTools')">
              <ToolbarIconButton v-if="can('assets.manage')" :icon="Upload" :label="t('common.import')" @click="openImportDialog" />
              <ToolbarIconButton v-if="can('assets.export')" :icon="Download" :label="t('common.export')" :loading="exportingAssets" :disabled="exportingAssets" @click="exportAssets" />
            </div>
          </template>
          <template #primary>
      ...
      <PageContent surface>
        <div v-if="selectedAssetIds.length" class="asset-selection-bar" role="region" :aria-label="t('asset.batchActions')">
          ...
          <div class="asset-selection-bar__actions">
            ...
            <el-button v-if="can('assets.manage')" type="primary" plain :disabled="assetBatchDeleteSaving || assetBatchAssignmentSaving || selectedAssetIds.length > 100" @click="openBulkEdit">{{ t('asset.bulkEdit') }}</el-button>
        ...
        <ResourceState :error="assetListError" @retry="searchLedger">
          ...
          >
            ...
            <el-table-column v-if="can('assets.manage')" :label="t('common.operation')" fixed="right" width="116">
              <template #default="{ row }">
    ...
    <div class="asset-filter-drawer">
      <el-form label-position="top">
        <el-form-item :label="t('asset.status')">
          <el-select v-model="advancedFilters.status" clearable :placeholder="t('asset.status')" class="asset-filter-drawer__control">
        ...
        <el-form-item :label="t('asset.deviceType')">
          <SearchableSelect
            v-model="advancedFilters.deviceType"
        ...
        <el-form-item :label="t('asset.tag')">
          <SearchableSelect
  ...
  <BulkEditAssetDialog
    ...
    :selected-assets="selectedAssetsForBatch"
    :request="context.request"
    :can="can"
    @completed="handleBulkEditCompleted"
    @unknown="handleBulkEditUnknown"
  />
