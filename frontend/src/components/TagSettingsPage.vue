...
const tagActiveDraft = ref("");
const activeTagFilterCount = computed(() => Number(Boolean(c.tagActive)));
...
function openTagFilters() {
  tagActiveDraft.value = c.tagActive;
  showTagFilters.value = true;
}

function clearTagFilters() {
  tagActiveDraft.value = "";
}

function applyTagFilters() {
  c.tagActive = tagActiveDraft.value;
  c.refreshTagList();
...
<template>
  <PageContainer>
    <template #toolbar>
      <PageToolbar class="settings-content-toolbar">
        ...
        <template #filters>
          <ToolbarIconButton :icon="Filter" :label="t('asset.moreFilters')" :badge="activeTagFilterCount" @click="openTagFilters" />
        </template>
        <template #primary>
    ...
    <PageContent surface>
      ...
      >
        ...
        <el-table-column v-if="c.can('tags.manage')" :label="t('common.operation')" width="116" fixed="right">
          <template #default="{ row }">
            <div class="ep-table-actions">
              ...
              />
                <TableIconButton :icon="row.is_active ? CircleClose : CircleCheck" :label="row.is_active ? t('status.inactive') : t('status.active')" :disabled="c.tagActionId === row.id" @click="c.toggleTag(row)" />
                <TableIconButton :icon="Delete" :label="t('common.delete')" type="danger" :disabled="c.tagActionId === row.id || (row.assets_count || 0) > 0" @click="c.deleteTag(row)" />
              </el-button-group>
            </div>
          </template>
        </el-table-column>
  ...
  <el-drawer v-model="showTagFilters" :title="t('asset.moreFilters')" size="360px" append-to-body>
    <el-form label-position="top">
      <el-form-item :label="t('tag.allStatuses')">
        <el-select v-model="tagActiveDraft" clearable :placeholder="t('tag.allStatuses')">
    ...
    <template #footer>
      <div class="page-filter-drawer__footer">
        <el-button @click="clearTagFilters">{{ t('common.clearFilters') }}</el-button>
        <el-button @click="showTagFilters = false">{{ t('common.cancel') }}</el-button>
        <el-button type="primary" @click="applyTagFilters">{{ t('asset.applyFilters') }}</el-button>
      </div>
    </template>
  </el-drawer>
  ...
  <FormDialogShell v-model="c.showTagModal" :title="c.editingTag ? t('tag.editTitle') : t('tag.createTitle')" :description="t('tag.description')" size="small" :saving="c.tagSaving" :show-close="!c.tagSaving" :close-disabled="c.tagSaving" :close-on-click-modal="!c.tagSaving" :close-on-press-escape="!c.tagSaving">
    ...
    <template #footer><el-button :disabled="c.tagSaving" @click="c.showTagModal = false">{{ t('common.cancel') }}</el-button><el-button type="primary" :loading="c.tagSaving" :disabled="c.tagSaving" @click="submitTag">{{ c.editingTag ? t('common.save') : t('common.create') }}</el-button></template>
  </FormDialogShell>
</template>
