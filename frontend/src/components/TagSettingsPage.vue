...
<template>
  <el-card shadow="never" class="settings-card">
    <template #header><div class="ep-toolbar settings-toolbar"><strong>标签管理</strong><SearchField v-model="c.tagSearch" placeholder="搜索标签" aria-label="搜索标签" @search="c.loadTags()" /><el-select v-model="c.tagActive" style="width: 120px" @change="c.loadTags()"><el-option label="全部状态" value="all" /><el-option label="启用" value="true" /><el-option label="停用" value="false" /></el-select><span class="ep-toolbar-spacer"></span><el-button type="primary" :disabled="!c.can('tags.manage')" @click="c.openTagModal()">新增标签</el-button></div></template>
