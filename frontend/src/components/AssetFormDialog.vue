...
function mapPerson(item: Record<string, unknown>): SearchableSelectOption {
  ...
  return { value: String(person.id), label: person.name || person.display_name, secondary: personMeta(person), data: person };
}
...
function mapDictionary(item: Record<string, unknown>): SearchableSelectOption {
  ...
  return { value: String(dictionary.id), label: dictionary.name, secondary: dictionary.code || "", data: dictionary };
}
...
function mapRack(item: Record<string, unknown>): SearchableSelectOption {
  ...
  return { value: String(rack.id), label: rack.code, secondary: [rack.name, rack.server_room_name, rack.data_center_name].filter(Boolean).join(" · "), data: rack };
}
...
<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="assetFormSaving" :disabled="assetFormLoading || !!assetFormLoadError || assetCustomSchemaLoading || !!assetCustomSchemaError" @click="submitAsset">
        {{ assetModalMode === 'edit' ? t('common.save') : t('common.create') }}
      </el-button>
    </template>
  </FormDialogShell>
  ...
  <FormDialogShell v-model="showQuickModel" :title="t('assetModel.createTitle')" :description="t('assetForm.quickAddModelHint')" size="small" :saving="quickModelSaving" :close-disabled="quickModelSaving">
    ...
    <template #footer><el-button :disabled="quickModelSaving" @click="showQuickModel = false">{{ t('common.cancel') }}</el-button><el-button type="primary" :loading="quickModelSaving" :disabled="!quickModelForm.name.trim() || !quickModelForm.manufacturer || !quickModelForm.device_type" @click="saveQuickModel">{{ t('common.create') }}</el-button></template>
  </FormDialogShell>
</template>
