...
import { CircleCheck, CircleClose, Delete, Edit, Filter } from "@element-plus/icons-vue";
...
const showCustomFieldFilters = ref(false);
const customFieldFilterDraft = reactive({ type: "", active: "" });
...
const activeCustomFieldFilterCount = computed(() =>
  Number(Boolean(c.customFieldType)) + Number(Boolean(c.customFieldActive)),
);

function openCustomFieldFilters() {
  customFieldFilterDraft.type = c.customFieldType;
  customFieldFilterDraft.active = c.customFieldActive;
  showCustomFieldFilters.value = true;
}

function clearCustomFieldFilters() {
  customFieldFilterDraft.type = "";
  customFieldFilterDraft.active = "";
}

function applyCustomFieldFilters() {
  c.customFieldType = customFieldFilterDraft.type;
  c.customFieldActive = customFieldFilterDraft.active;
  c.refreshCustomFieldList();
  showCustomFieldFilters.value = false;
}
...
<template>
  <PageContainer>
    <template #toolbar>
      <PageToolbar class="settings-content-toolbar">
        <template #search>
        ...
        <template #filters>
          <ToolbarIconButton :icon="Filter" :label="t('asset.moreFilters')" :badge="activeCustomFieldFilterCount" @click="openCustomFieldFilters" />
        </template>
        <template #primary>
    ...
    <PageContent surface>
      ...
      >
        ...
        <el-table-column v-if="c.can('custom_fields.manage')" :label="t('common.operation')" width="116" fixed="right">
          <template #default="{ row }">
            <div class="ep-table-actions">
              ...
              />
                <TableIconButton :icon="row.is_active ? CircleClose : CircleCheck" :label="row.is_active ? t('status.inactive') : t('status.active')" :disabled="c.customFieldActionId === row.id" @click="c.toggleCustomField(row)" />
                <TableIconButton :icon="Delete" :label="t('common.delete')" type="danger" :disabled="c.customFieldActionId === row.id || (row.assets_count || 0) > 0 || (row.fieldsets_count || 0) > 0" @click="c.deleteCustomField(row)" />
              </el-button-group>
            </div>
          </template>
        </el-table-column>
  ...
  <el-drawer v-model="showCustomFieldFilters" :title="t('asset.moreFilters')" size="360px" append-to-body>
    <el-form label-position="top">
      <el-form-item :label="t('common.type')">
        <el-select v-model="customFieldFilterDraft.type" clearable :placeholder="t('common.type')">
      ...
      <el-form-item :label="t('customField.allStatuses')">
        <el-select v-model="customFieldFilterDraft.active" clearable :placeholder="t('customField.allStatuses')">
    ...
    <template #footer>
      <div class="page-filter-drawer__footer">
        <el-button @click="clearCustomFieldFilters">{{ t('common.clearFilters') }}</el-button>
        <el-button @click="showCustomFieldFilters = false">{{ t('common.cancel') }}</el-button>
        <el-button type="primary" @click="applyCustomFieldFilters">{{ t('asset.applyFilters') }}</el-button>
      </div>
    </template>
  </el-drawer>
  ...
  <FormDialogShell v-model="c.showCustomFieldModal" class="custom-field-dialog" :title="c.editingCustomField ? t('customField.editTitle') : t('customField.createTitle')" :description="t('customField.description')" size="large" :saving="c.customFieldSaving" :show-close="!c.customFieldSaving" :close-disabled="c.customFieldSaving" :close-on-click-modal="!c.customFieldSaving" :close-on-press-escape="!c.customFieldSaving">
    ...
    <template #footer><el-button :disabled="c.customFieldSaving" @click="c.showCustomFieldModal = false">{{ t('common.cancel') }}</el-button><el-button type="primary" :loading="c.customFieldSaving" :disabled="c.customFieldSaving" @click="submitCustomField">{{ c.editingCustomField ? t('common.save') : t('common.create') }}</el-button></template>
  </FormDialogShell>
  ...
  <FormDialogShell v-model="c.showCustomFieldOptionModal" class="custom-field-option-dialog" :title="t('customField.optionTitle') + (c.editingCustomField ? `: ${c.editingCustomField.name}` : '')" :description="t('customField.optionDescription')" size="medium" :saving="c.customFieldOptionSaving" :show-close="!c.customFieldOptionSaving" :close-disabled="c.customFieldOptionSaving" :close-on-click-modal="!c.customFieldOptionSaving" :close-on-press-escape="!c.customFieldOptionSaving">
    <section class="form-dialog__section">
      ...
      >
        <el-table v-loading="c.customFieldOptionLoading" :data="c.editingCustomField?.options || []" size="small">
          ...
          <el-table-column :label="t('common.operation')" width="84">
            <template #default="{ row }">
              <div class="ep-table-actions">
                <el-button-group>
                <TableIconButton
                ...
                />
                <TableIconButton :icon="Delete" :label="t('common.delete')" type="danger" :disabled="c.customFieldOptionActionId === row.id || c.customFieldOptionSaving" @click="c.deleteCustomFieldOption(row)" />
                </el-button-group>
              </div>
            </template>
          </el-table-column>
    ...
    <template #footer><el-button :disabled="c.customFieldOptionSaving" @click="c.showCustomFieldOptionModal = false">{{ t('common.cancel') }}</el-button><el-button type="primary" :loading="c.customFieldOptionSaving" :disabled="c.customFieldOptionSaving" @click="submitCustomFieldOption">{{ c.editingCustomFieldOption ? t('common.save') : t('common.create') }}</el-button></template>
  </FormDialogShell>
</template>
