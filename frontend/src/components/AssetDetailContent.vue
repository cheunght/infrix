...
import AssetAuditHistory from "./AssetAuditHistory.vue";
...
const props = withDefaults(
  defineProps<{
    ...
    auditHistoryContext?: AssetAuditHistoryContext | null;
    attachmentContext?: AssetAttachmentContext | null;
  }>(),
  {
    variant: "drawer",
...
const { asset, loading, error, showSummary, descriptionColumns } =
  toRefs(props);
const { t } = useI18n();
...
const responsibilityHistoryContext =
  computed<AssetResponsibilityHistoryContext | null>(
...
const auditHistoryContext = computed<AssetAuditHistoryContext | null>(
  () => props.auditHistoryContext || null,
);
const showResponsibilityHistory = computed(
  () =>
    responsibilityHistoryContext.value?.responsibilityHistoryCanView.value ===
    true,
);
const showInventoryHistory = computed(
  () => inventoryHistoryContext.value?.inventoryHistoryCanView.value === true,
);
const showAuditHistory = computed(
  () => auditHistoryContext.value?.assetAuditCanView.value === true,
);
...
type ProcurementRecord = NonNullable<
  AssetDetail["procurement_records"]
>[number];
type MaintenanceRecord = NonNullable<
  AssetDetail["maintenance_contracts"]
>[number];
...
function inventoryRecordLabel(record: InventoryItem): string {
  const itemStatusLabel = businessOptionLabel(
    INVENTORY_ITEM_STATUS_OPTIONS,
    record.status,
  );
  const resolution =
...
function roleLabel(role: string): string {
  return (
    (
      {
        business: t("asset.businessIp"),
        management: t("asset.managementIp"),
        oob: t("asset.oobIp"),
...
const basicFields = computed<DetailField[]>(() => {
  ...
  const fields = isDrawer.value
    ? [
        makeField(
          "device-type",
          t("asset.deviceType"),
          current.device_type_name,
        ),
        makeField(
          "manufacturer",
          t("asset.manufacturer"),
  ...
  if (!isDrawer.value && !props.responsibilityContext) {
    fields.push(
      makeField(
        "assigned-person",
        t("asset.assignedPerson"),
        current.assigned_person?.display_name,
      ),
    );
  }
  return fields;
...
const locationFields = computed<DetailField[]>(() => {
  ...
  const dataCenter =
    rack?.data_center || current.asset_data_center_name || current.data_center;
  if (isDrawer.value) {
    const fields = [
      {
        ...makeField(
          ...
          [
            dataCenter,
            rack?.server_room || current.server_room,
            rack?.rack_code || current.rack_code,
          ]
            .filter(hasContent)
            .join(" / "),
        ),
      ...
      makeField(
        "location-status",
        t("asset.locationStatus"),
        rack ? t("asset.mounted") : t("asset.unmounted"),
      ),
      makeField("u-range", t("asset.uRange"), rackPositionLabel(current)),
...
const networkRows = computed<AssetNetwork[]>(
  () => asset.value?.network_addresses || [],
);
...
const procurementRecords = computed(
  () => asset.value?.procurement_records || [],
);
const maintenanceRecords = computed(
  () => asset.value?.maintenance_contracts || [],
);
const procurementRecordCount = computed(() => {
  if (procurementRecords.value.length) return procurementRecords.value.length;
  const current = asset.value;
  return current &&
    [current.purchase_date, current.supplier, current.purchase_order_no].some(
      hasContent,
    )
    ? 1
    : 0;
...
function procurementRecordFields(
  ...
  return [
    makeField(
      ...
      record?.purchase_date || current?.purchase_date,
      formatDate,
    ),
    makeField(
      "supplier",
      t("asset.supplier"),
      record?.supplier || current?.supplier,
    ),
    makeField(
      "purchase-order",
      t("asset.purchaseOrder"),
      record?.order_no || current?.purchase_order_no,
    ),
    makeField(
      "purchase-amount",
      t("asset.purchaseAmount"),
      record?.amount,
      formatMoneyDecimalString,
    ),
    {
...
function maintenanceStateFor(expiry: unknown): MaintenanceState | null {
  ...
  if (
    ...
    !todayStart ||
    Number.isNaN(todayStart.getTime())
  )
    return null;
  const daysRemaining = Math.round(
    (expiryDate.getTime() - todayStart.getTime()) / 86400000,
  );
  if (daysRemaining < 0)
    return { label: t("status.expired"), status: "expired", type: "danger" };
  if (daysRemaining <= 30)
    return { label: t("status.expiring"), status: "expiring", type: "warning" };
...
function maintenanceRecordFields(
  ...
  return [
    makeField(
      ...
      t("asset.maintenanceProvider"),
      record?.provider,
    ),
    makeField(
      "maintenance-contract",
      t("asset.maintenanceContract"),
      record?.contract_no,
    ),
    makeField(
      "maintenance-start",
      t("asset.maintenanceStart"),
      record?.start_date,
      formatDate,
    ),
    makeField(
      "maintenance-expiry",
      t("asset.maintenanceExpiry"),
      record?.expiry_date,
      formatDate,
    ),
    makeField(
      "maintenance-status",
      t("asset.maintenanceStatus"),
      record?.expiry_date,
...
const depreciationStatus = computed(
  () => depreciation.value?.status || "unconfigured",
);
...
const depreciationFields = computed<DetailField[]>(() => {
  ...
  return [
    makeField(
      "depreciation-status",
      t("asset.depreciationStatus"),
      depreciationStatus.value,
      (value) => depreciationStatusLabel(String(value)),
    ),
    makeField(
      "depreciation-method",
      t("asset.depreciationMethod"),
      current?.method,
      (value) => depreciationMethodLabel(String(value)),
    ),
    makeField(
      "depreciation-start",
      t("asset.depreciationStart"),
      current?.start_date,
      formatDate,
    ),
    makeField(
      "depreciation-years",
      t("asset.depreciationYears"),
      current?.years,
      (value) => `${value} ${t("common.years")}`,
    ),
    makeField(
      "depreciation-rate",
      t("asset.residualRate"),
      current?.residual_rate,
      formatResidualRate,
    ),
    makeField(
      "depreciation-original",
      t("asset.originalValue"),
      current?.original_value,
      formatMoneyDecimalString,
    ),
    makeField(
      "depreciation-residual-value",
      t("asset.estimatedResidual"),
    ...
    makeField(
      ...
      current?.monthly_depreciation,
      formatMoneyDecimalString,
    ),
    makeField(
      "depreciation-accumulated",
      t("asset.accumulatedDepreciation"),
      current?.accumulated_depreciation,
      formatMoneyDecimalString,
    ),
    makeField(
      "depreciation-net",
      t("asset.netBookValue"),
      current?.net_book_value,
      formatMoneyDecimalString,
    ),
    makeField(
      "depreciation-elapsed-months",
      t("asset.elapsedMonths"),
      current?.elapsed_months,
      (value) => `${value} ${t("common.months")}`,
    ),
    makeField(
      "depreciation-total-months",
      t("asset.totalMonths"),
      current?.total_months,
      (value) => `${value} ${t("common.months")}`,
    ),
    makeField(
      "depreciation-progress",
      t("asset.depreciationProgress"),
      current?.progress,
      (value) =>
        formatDepreciationProgress(
          value,
...
const customFieldCount = computed(() =>
  currentFieldGroups.value.reduce(
...
const latestInventoryRecord = computed(
  () => asset.value?.latest_inventory_record || null,
);
const relatedRecordFields = computed<DetailField[]>(() => [
  makeField(
    "inventory-count",
    t("asset.inventoryRecords"),
    inventoryRecordCount.value,
    (value) => t("units.item", Number(value)),
  ),
  makeField(
    "latest-inventory",
    t("asset.recentInventory"),
    latestInventoryRecord.value?.checked_at,
    formatDateTime,
  ),
]);
...
<template>
  ...
  >
    <div
      v-if="loading"
      class="drawer-state asset-detail-state"
      role="status"
      aria-live="polite"
    >
    ...
    <div
      v-else-if="error"
      class="drawer-state drawer-error asset-detail-state"
      role="alert"
    >
      <strong>{{ t("asset.assetDetailLoadFailed") }}</strong>
      <span>{{ error }}</span>
      <el-button v-if="retry" type="primary" plain @click="retryDetail">{{
        t("common.retry")
      }}</el-button>
    </div>
    <template v-else-if="asset">
      <div v-if="showSummary !== false" class="asset-detail-summary">
        <div>
          ...
          <strong :title="displayValue(asset.name)">{{
            displayValue(asset.name)
          }}</strong>
        </div>
        <div>
          <span>{{ t("asset.code") }}</span>
          <strong :title="displayValue(asset.asset_no)">{{
            displayValue(asset.asset_no)
          }}</strong>
        </div>
        <div>
      ...
      <DetailSection
        :title="t(isDrawer ? 'asset.deviceInfo' : 'asset.basicInfo')"
      >
        <DescriptionList
      ...
      <DetailSection :title="t('asset.locationOwnership')">
        ...
        <AssetResponsibilityActions
          v-if="isDrawer && responsibilityActionContext"
          :asset="asset"
      ...
      <DetailSection :title="t('asset.network')">
        <div v-if="isDrawer && networkRows.length" class="asset-network-list">
          <article
            v-for="network in networkRows"
            :key="network.id"
            class="asset-network-row"
          >
            <span>{{ displayValue(roleLabel(network.role)) }}</span>
            <span class="asset-detail-technical">{{
              displayValue(network.address)
            }}</span>
            <span class="asset-network-row__flags">
              <el-tag v-if="network.is_primary" size="small">{{
                t("asset.networkPrimary")
              }}</el-tag>
              <span v-else>{{ t("common.no") }}</span>
              <span>{{ networkStatusLabel(network.status) }}</span>
            </span>
            <p v-if="hasContent(network.notes)">{{ network.notes }}</p>
          </article>
        </div>
        <p v-else-if="isDrawer" class="asset-detail-footnote">
          {{ t("asset.noNetworkRecords") }}
        </p>
        <el-table
          v-else-if="networkRows.length"
          :data="networkRows"
        ...
        >
          ...
          <el-table-column
            prop="address"
            :label="t('asset.networkAddress')"
            min-width="150"
            show-overflow-tooltip
          />
          <el-table-column
            prop="is_primary"
            :label="t('asset.networkPrimary')"
            width="90"
          >
          ...
          <el-table-column
            prop="status"
            :label="t('asset.networkStatus')"
            width="90"
          >
            <template #default="{ row }">{{
              networkStatusLabel(row.status)
            }}</template>
          </el-table-column>
          <el-table-column
            prop="notes"
            :label="t('asset.networkNotes')"
            min-width="160"
            show-overflow-tooltip
          />
        </el-table>
      ...
      <DetailSection v-if="isDrawer && attachmentContext?.canView" :title="t('attachment.title')" :collapsible="false">
        <AttachmentSection
          :asset-id="asset.id"
          scope="asset"
          :request="attachmentContext.request"
          :download="attachmentContext.download"
          :confirm-action="attachmentContext.confirmAction"
          :can-view="attachmentContext.canView"
          :can-manage="attachmentContext.canManage"
          :show-heading="false"
        />
      </DetailSection>
      ...
      <AssetResponsibilityHistory
        v-if="
          !isDrawer && responsibilityHistoryContext && showResponsibilityHistory
        "
        :context="responsibilityHistoryContext"
        :collapsible="isDrawer"
      />
      <template
        v-for="group in isDrawer
          ? ['supplement', 'financial']
          : ['financial', 'supplement']"
        :key="group"
      >
        <template v-if="group === 'supplement'">
          <DetailSection
            v-if="!isDrawer || tags.length"
            :title="t('asset.tags')"
            :collapsible="false"
          ...
          >
            <div v-if="tags.length" class="asset-detail-tags">
              ...
              >
                {{ tag.name
                }}<template v-if="!tag.is_active">
                  ({{ t("status.inactive") }})</template
                >
              </el-tag>
            </div>
            <DescriptionList
          ...
          <DetailSection
            v-if="!isDrawer || customFieldCount"
            :title="t('asset.customFields')"
            :collapsible="false"
            :summary="itemCount(customFieldCount)"
          >
            <div
              v-if="currentFieldGroups.length"
              class="asset-detail-custom-groups"
            >
              <section
                v-for="group in currentFieldGroups"
                :key="group.name"
                class="asset-detail-custom-group"
              >
                <h4>{{ group.name }}</h4>
                <DescriptionList
                  class="asset-detail-description-list"
            ...
            <DescriptionList
              ...
              class="asset-detail-description-list"
              :items="[
                makeField('custom-fields-empty', t('asset.customFields'), null),
              ]"
              :columns="descriptionColumns"
              :layout="descriptionLayout"
            />
          ...
          <DetailSection
            v-if="!isDrawer || hasContent(asset.notes)"
            :title="t('common.notes')"
          >
            <p
              :class="[
                'detail-notes',
                { 'asset-detail-empty-value': !hasContent(asset.notes) },
              ]"
            >
        ...
        <template v-else>
          ...
          >
            <div
              v-if="procurementRecords.length"
              class="asset-detail-record-list"
            >
              <article
            ...
            <p
              v-else-if="isDrawer && !procurementRecordCount"
              class="asset-detail-footnote"
            >
              {{ t("asset.noProcurementRecords") }}
            </p>
            <DescriptionList
              v-else
              class="asset-detail-description-list"
          ...
          >
            <div
              v-if="maintenanceRecords.length"
              class="asset-detail-record-list"
            >
              <article
              ...
              >
                ...
                >
                  <template #value-maintenance-status="{ item }">
                    ...
                    />
                    <span v-else class="asset-detail-empty-value">{{
                      item.value
                    }}</span>
                  </template>
                </DescriptionList>
          ...
          >
            <p
              v-if="isDrawer && depreciationStatus === 'unconfigured'"
              class="asset-detail-footnote"
            >
              {{ depreciationStatusLabel(depreciationStatus) }}
            </p>
            <DescriptionList
              v-else
              class="asset-detail-description-list"
      ...
      <DetailSection v-if="!isDrawer" :title="t('asset.systemInfo')">
        <DescriptionList
          class="asset-detail-description-list"
          :items="systemFields"
          :columns="descriptionColumns"
      ...
      <AssetResponsibilityHistory
        v-if="
          isDrawer && responsibilityHistoryContext && showResponsibilityHistory
        "
        class="asset-detail-responsibility-history"
        compact
        :context="responsibilityHistoryContext"
        collapsible
      />
      ...
      <DetailSection
        v-if="!isDrawer || !showInventoryHistory || !inventoryHistoryContext"
        :class="{ 'asset-detail-history': isDrawer }"
        :title="
          t(
            isDrawer && showInventoryHistory
              ? 'asset.inventoryRecords'
              : 'asset.relatedRecords',
          )
        "
        :collapsible="isDrawer"
        ...
        >
          <template #value-latest-inventory="{ item }">
            <span class="asset-detail-inline-value">
              ...
              />
              <span v-if="latestInventoryRecord">{{
                formatDateTime(latestInventoryRecord.checked_at)
              }}</span>
              <span v-else class="asset-detail-empty-value">{{
                item.value
              }}</span>
      ...
      >
        <DescriptionList
          v-if="isDrawer && inventoryRecordCount > 0"
          :items="relatedRecordFields.slice(1)"
          :columns="1"
          layout="compact"
        >
          <template #value-latest-inventory="{ item }">
      ...
      <footer v-if="isDrawer" class="asset-detail-footer">
        <span v-for="field in systemFields" :key="field.key"
