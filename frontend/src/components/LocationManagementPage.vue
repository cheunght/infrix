...
<template>
    ...
    >
      ...
      >
        ...
        <el-table-column :label="t('common.operation')" column-key="actions" width="152" fixed="right">
          <template #default="{ row }">
            <div class="ep-table-actions" @click.stop>
              <template v-if="row.nodeType === 'data-center'">
                <template v-if="can('racks.manage')">
                  <el-button-group>
                    <TableIconButton :icon="Edit" :label="t('common.edit')" type="primary" :disabled="dataCenterActionId === row.id" @click="context.openDataCenterModal(findDataCenter(row) || undefined)" />
                    <TableIconButton :icon="FolderAdd" :label="t('location.createRoom')" :disabled="dataCenterActionId === row.id" @click="handleDataCenterCommand(row, 'new-room')" />
                    <TableIconButton :icon="row.is_active ? CircleClose : CircleCheck" :label="row.is_active ? t('status.inactive') : t('status.active')" :disabled="dataCenterActionId === row.id" @click="handleDataCenterCommand(row, row.is_active ? 'disable' : 'enable')" />
                    <TableIconButton :icon="Delete" :label="t('common.delete')" type="danger" :disabled="dataCenterActionId === row.id || dataCenterHasAssociations(row)" @click="handleDataCenterCommand(row, 'delete')" />
                  </el-button-group>
                </template>
                <span v-else>—</span>
              </template>
              <template v-else>
                <el-button-group>
                  <TableIconButton :icon="Grid" :label="t('location.viewRacks')" type="primary" :disabled="updatingRoomId === row.id" @click="handleRoomCommand(row, 'view-racks')" />
                  <template v-if="can('racks.manage')">
                    <TableIconButton :icon="Edit" :label="t('common.edit')" type="primary" :disabled="updatingRoomId === row.id" @click="handleRoomCommand(row, 'edit')" />
                    <TableIconButton :icon="row.is_active ? CircleClose : CircleCheck" :label="row.is_active ? t('status.inactive') : t('status.active')" :disabled="updatingRoomId === row.id" @click="handleRoomCommand(row, row.is_active ? 'disable' : 'enable')" />
                    <TableIconButton :icon="Delete" :label="t('common.delete')" type="danger" :disabled="updatingRoomId === row.id || roomHasAssociations(row)" @click="handleRoomCommand(row, 'delete')" />
                  </template>
                </el-button-group>
              </template>
            </div>
