...
import { CircleCheck, CircleClose, Delete, Download, Edit, Filter, InfoFilled, Refresh, View } from "@element-plus/icons-vue";
...
import { routeForPage, type MaintenanceTab, type SystemSettingsTab } from "../router";
import type { AuditLog, Department, PersonOption, SystemSettingsForm } from "../types";
...
type SystemOperationsHandle = {
  busy: boolean;
  load: () => Promise<void>;
};

type BackupRestorePageHandle = {
  loading: boolean;
  creating: boolean;
  restoring: boolean;
  loadBackups: () => Promise<boolean>;
...
const systemOperationsRef = ref<SystemOperationsHandle | null>(null);
const backupRestoreRef = ref<BackupRestorePageHandle | null>(null);
...
const systemOperationsBusy = computed(() => systemOperationsRef.value?.busy ?? true);
const backupLoading = computed(() => backupRestoreRef.value?.loading ?? true);
const backupCreating = computed(() => backupRestoreRef.value?.creating ?? false);
const backupRestoring = computed(() => backupRestoreRef.value?.restoring ?? false);
...
function saveBranding(reset = false) {
  if (!brandingSettingsRef.value) return;
  void brandingSettingsRef.value.save(reset);
}

function refreshSystemOperations() {
  if (!systemOperationsRef.value) return;
  void systemOperationsRef.value.load();
}

function refreshBackups() {
  if (!backupRestoreRef.value) return;
  void backupRestoreRef.value.loadBackups();
}

function createBackup() {
  if (!backupRestoreRef.value) return;
  void backupRestoreRef.value.createBackup();
}
...
function maintenanceTabFromRoute(): MaintenanceTab {
  ...
  if (context.can("system.reset") && (routeTab === "backup" || routeTab === "reset")) return routeTab;
  return "operations";
}

function syncMaintenanceTabFromRoute() {
  maintenanceTab.value = context.settingsSection.value === "maintenance"
    ? maintenanceTabFromRoute()
    : "operations";
  if (context.settingsSection.value !== "maintenance") return;

  const routeTab = Array.isArray(route.query.tab) ? route.query.tab[0] : route.query.tab;
  const isKnownTab = routeTab === undefined || routeTab === "operations" || routeTab === "backup" || routeTab === "reset";
  const isUnauthorizedTab = !context.can("system.reset") && (routeTab === "backup" || routeTab === "reset");
  if (!isKnownTab || isUnauthorizedTab) {
    void router.replace(routeForPage("settings", { settingsSection: "maintenance" }));
  }
}

watch(
  [context.settingsSection, () => context.can("system.reset"), () => route.query.tab],
...
function changeMaintenanceTab(value: string) {
  ...
  void router.push(routeForPage("settings", {
    ...
    maintenanceTab: maintenanceTab.value,
  }));
}
const {
  settingsSection,
...
const currentSystemSettingsKeys = computed<Array<keyof SystemSettingsForm>>(() => {
  if (systemSettingsTab.value === "branding") return [];
  const keys = systemSettingsDefinitions.value
    .filter((definition) => definition.section === systemSettingsTab.value)
    .map((definition) => definition.key as keyof SystemSettingsForm);
  if (systemSettingsTab.value === "smtp") keys.push("smtp_password");
  return keys;
});
...
function resetCurrentSystemSettingsForm() {
  resetSystemSettingsForm(currentSystemSettingsKeys.value);
}
function saveCurrentSystemSettings() {
  return saveSystemSettings(currentSystemSettingsKeys.value);
}
...
const activePeopleFilterCount = computed(() =>
  Number(Boolean(responsibilityDirectoryType.value)) +
  Number(responsibilityDirectoryActive.value !== "true"),
);
...
<template>
  <div class="infrix-page settings-page">
          ...
          <template #primary>
            <div v-if="systemSettingsTab !== 'branding'" class="settings-system__actions">
              ...
              <el-button
                ...
                :disabled="!currentSystemSettingsDirty || systemSettingsSaving"
                @click="saveCurrentSystemSettings"
              >
                {{ t('settings.saveSettings') }}
              </el-button>
            </div>
            <div v-else class="settings-system__actions">
              <el-button :disabled="brandingBusy || !brandingReady || !can('settings.manage')" @click="saveBranding(true)">{{ t('branding.restore') }}</el-button>
              <el-button
                type="primary"
                :disabled="brandingBusy || !brandingReady || !can('settings.manage')"
                @click="saveBranding()"
              >
                {{ t('branding.save') }}
              </el-button>
        ...
        <div v-loading="systemSettingsLoading" class="settings-system">
          ...
          <template v-else-if="systemSettings">
            ...
            <el-form label-position="top" @submit.prevent="saveCurrentSystemSettings">
                ...
                <div class="settings-system__notification-sections">
                  <section class="settings-system__notification-section">
                    <header class="settings-system__notification-section-heading settings-system__notification-section-heading--with-control">
                      ...
                      <div class="settings-system__notification-section-switch">
                        <el-form-item class="settings-system__notification-form-item" :error="systemSettingsFormErrors.email_digest_enabled">
                          <el-switch
                            v-model="systemSettingsForm.email_digest_enabled"
                            :disabled="!can('settings.manage') || systemSettingsSaving"
                            :aria-label="t('operations.emailEnabled')"
                          />
                        </el-form-item>
                        <span class="settings-system__notification-switch-label">
                          {{ systemSettingsForm.email_digest_enabled ? t('status.enabled') : t('status.disabled') }}
                        </span>
                      </div>
                    </header>
                    <div class="settings-system__digest-grid">
                      ...
                      <el-form-item class="settings-system__digest-full-width" :label="t('operations.applicationUrl')" :error="systemSettingsFormErrors.application_url">
                  ...
                  <section class="settings-system__notification-section">
                    ...
                    <div class="settings-system__notification-list">
                      ...
                      <div class="settings-system__notification-row settings-system__notification-row--without-field">
              ...
              <BrandingSettings v-else-if="systemSettingsTab === 'branding'" ref="brandingSettingsRef" :context="context" />
            </el-form>
          </template>
    ...
    <PageContainer v-else-if="settingsSection === 'dictionaries'">
      ...
      <PageContent surface>
        ...
        >
          ...
          <el-table-column v-if="canManageCurrentDictionary" :label="t('common.operation')" width="116" fixed="right">
            <template #default="{ row }">
    ...
    <PageContainer v-else-if="settingsSection === 'organization' && (can('organization.manage') || can('settings.manage') || can('settings.view'))" :key="'organization-settings'">
      <template #subnav>
        <PageTabs v-if="organizationTabs.length" :key="organizationTabsKey" v-model="organizationTab" :items="organizationTabs" @update:model-value="changeOrganizationTab" />
      </template>
      <PageContent v-if="organizationTab === 'ldap'" surface>
        ...
        <LdapConfigurationPage :context="props.context">
          <template #notice>
      ...
      <PageContent v-else surface>
        ...
        <PeopleSettingsPage v-if="organizationTab === 'people'" :context="props.context" />
        <template v-else-if="organizationTab === 'departments'">
          ...
          >
            <el-table v-loading="departmentLoading" :data="departments" table-layout="fixed">
              ...
              <el-table-column v-if="can('settings.manage')" :label="t('common.operation')" width="84" fixed="right">
                <template #default="{ row }">
                  <div class="ep-table-actions">
                    <el-button-group>
        ...
        <template v-else-if="organizationTab === 'users'">
          ...
          <el-table ref="userTableRef" v-else v-loading="organizationLoading" :data="users" table-layout="fixed" @selection-change="handleUserSelection">
            ...
            <el-table-column v-if="can('organization.manage')" :label="t('common.operation')" width="152" fixed="right">
              <template #default="{ row }">
                <div class="ep-table-actions">
                  <el-button-group>
                  <TableIconButton
                  ...
                  />
                    <TableIconButton :icon="row.is_active ? CircleClose : CircleCheck" :label="userProtectionReason(row) || (row.is_active ? t('status.inactive') : t('status.active'))" :disabled="userPendingId === row.id || userSaving || Boolean(userProtectionReason(row))" @click="toggleUser(row)" />
                    <TableIconButton :icon="Delete" :label="userDeleteProtectionReason(row) || t('settings.deleteUser')" type="danger" :disabled="userPendingId === row.id || userSaving || Boolean(userDeleteProtectionReason(row))" @click="deleteUser(row)" />
                  </el-button-group>
                </div>
              </template>
            </el-table-column>
    ...
    <el-drawer v-if="organizationTab === 'people'" v-model="showPeopleFilters" :title="t('asset.moreFilters')" size="360px" append-to-body>
      <el-form label-position="top">
        <el-form-item :label="t('settings.personDepartment')">
          <SearchableSelect
            v-model="peopleFilterDraft.department"
            :request="context.request"
            endpoint="/departments/"
            :map-option="mapDepartment"
            :base-query="{ is_active: 'all' }"
            :selected-option="selectedPeopleDepartmentOption"
            :placeholder="t('settings.personDepartment')"
            clearable
          />
        </el-form-item>
        <el-form-item :label="t('common.status')">
          <el-select v-model="peopleFilterDraft.active" :placeholder="t('common.all')" clearable>
      ...
      <template #footer>
        <div class="page-filter-drawer__footer">
          <el-button @click="clearPeopleFilters">{{ t('common.clearFilters') }}</el-button>
          <el-button @click="showPeopleFilters = false">{{ t('common.cancel') }}</el-button>
          <el-button type="primary" @click="applyPeopleFilters">{{ t('asset.applyFilters') }}</el-button>
        </div>
      </template>
    </el-drawer>
    ...
    <PageContainer v-else-if="settingsSection === 'maintenance' && can('settings.view')" content-class="settings-maintenance-container">
      ...
      <PageContent surface>
        <PageToolbar v-if="maintenanceTab === 'operations' || (maintenanceTab === 'backup' && can('system.reset'))" class="settings-form-toolbar">
          <template #actions>
            <ToolbarIconButton
              v-if="maintenanceTab === 'operations'"
              :icon="Refresh"
              :label="t('common.refresh')"
              :loading="systemOperationsBusy"
              :disabled="systemOperationsBusy"
              @click="refreshSystemOperations"
            />
            <template v-else-if="maintenanceTab === 'backup' && can('system.reset')">
              <ToolbarIconButton :icon="Refresh" :label="t('common.refresh')" :loading="backupLoading" :disabled="backupLoading || backupCreating || backupRestoring" @click="refreshBackups" />
              <el-button type="primary" :loading="backupCreating" :disabled="backupLoading || backupCreating || backupRestoring" @click="createBackup">
        ...
        <div class="settings-maintenance">
          <SystemOperations v-show="maintenanceTab === 'operations'" ref="systemOperationsRef" :context="context" />
          <BackupRestorePage v-if="maintenanceTab === 'backup' && can('system.reset')" ref="backupRestoreRef" :context="context" />
          <el-alert
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="departmentSaving" :disabled="departmentSaving" @click="saveDepartment">{{ editingDepartment ? t('common.save') : t('common.create') }}</el-button>
      </template>
    </el-dialog>
