...
type LabelTemplateKey = "standard" | "compact" | "location";
type LabelFieldKey = "assetNo" | "name" | "model" | "serial" | "location";

const LABEL_TEMPLATE_FIELDS: Record<LabelTemplateKey, readonly LabelFieldKey[]> = {
  standard: ["assetNo", "name", "model", "serial"],
  compact: ["assetNo", "name", "model"],
  location: ["assetNo", "name", "location", "serial"],
};
...
const error = ref("");
const storedLabelSize = typeof window === "undefined" ? "" : window.localStorage.getItem("infrix.asset-label-size");
const storedLabelTemplate = typeof window === "undefined" ? "" : window.localStorage.getItem("infrix.asset-label-template");
const labelSize = ref<"small" | "medium">(storedLabelSize === "small" ? "small" : "medium");
const labelTemplate = ref<LabelTemplateKey>(
  storedLabelTemplate === "compact" || storedLabelTemplate === "location" ? storedLabelTemplate : "standard",
);
let generation = 0;

const labelTemplateFields = computed(() => LABEL_TEMPLATE_FIELDS[labelTemplate.value]);

watch([labelSize, labelTemplate], () => {
  if (typeof window === "undefined") return;
  window.localStorage.setItem("infrix.asset-label-size", labelSize.value);
  window.localStorage.setItem("infrix.asset-label-template", labelTemplate.value);
});
...
function assetNameLabel(asset: Asset): string {
  return asset.name || t("asset.unlisted");
}

function labelFieldValue(asset: Asset, field: LabelFieldKey): string {
  if (field === "assetNo") return asset.asset_no || `#${asset.id}`;
  if (field === "name") return assetNameLabel(asset);
  if (field === "model") return modelLabel(asset);
  if (field === "serial") return serialLabel(asset);
  return assetLocationLabel(asset);
}

function labelFieldClass(field: LabelFieldKey): string {
  return `asset-qr-label__field--${field}`;
}
  ...
  const labels = entries.value.map((entry) => `
    <article class="asset-qr-print-label">
      ...
      <div class="asset-qr-print-copy">
        ${labelTemplateFields.value.map((field) => `<span class="asset-qr-print-field asset-qr-print-field--${field}">${escapeHtml(labelFieldValue(entry.asset, field))}</span>`).join("")}
      </div>
    </article>`).join("");
      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; }
          ...
          .asset-qr-print-field { overflow-wrap: anywhere; word-break: break-word; line-height: 1.15; }
          .asset-qr-print-field--assetNo { font-size: ${isSmall ? "9pt" : "11pt"}; font-weight: 700; }
          .asset-qr-print-field--name { font-size: ${isSmall ? "7pt" : "9pt"}; font-weight: 600; }
          .asset-qr-print-field--model, .asset-qr-print-field--location { color: #4b5563; font-size: ${isSmall ? "6pt" : "8pt"}; }
          .asset-qr-print-field--serial { color: #606266; font-size: ${isSmall ? "6pt" : "7pt"}; }
  ...
  >
    ...
    <div v-loading="loading" class="asset-qr-dialog__body">
      ...
      <div v-if="entries.length" class="asset-qr-dialog__options">
        <span>{{ t('asset.labelTemplate') }}</span>
        <el-select v-model="labelTemplate" size="small" :aria-label="t('asset.labelTemplate')" style="width: 150px">
          <el-option :label="t('asset.standardLabelTemplate')" value="standard" />
          <el-option :label="t('asset.compactLabelTemplate')" value="compact" />
          <el-option :label="t('asset.locationLabelTemplate')" value="location" />
        </el-select>
      ...
      <div v-if="entries.length" class="asset-qr-sheet" :class="[`asset-qr-sheet--${labelSize}`, { 'asset-qr-sheet--single': entries.length === 1 }]">
        <article v-for="entry in entries" :key="entry.asset.id" class="asset-qr-label">
          ...
          <div class="asset-qr-label__copy">
            <span
              v-for="field in labelTemplateFields"
              :key="field"
              class="asset-qr-label__field"
              :class="labelFieldClass(field)"
              :title="labelFieldValue(entry.asset, field)"
            >{{ labelFieldValue(entry.asset, field) }}</span>
          </div>
    ...
    <template #footer>
      ...
      <el-button type="primary" :disabled="!entries.length || loading" @click="printEntries">
        {{ t('asset.printLabels') }}
      </el-button>
    </template>
  </el-dialog>
...
.asset-qr-label__field--assetNo {
  color: var(--el-text-color-primary);
  font-size: 16px;
  font-weight: 700;
}

.asset-qr-label__field--name {
  color: var(--el-text-color-regular);
  font-size: 14px;
  font-weight: 600;
}

.asset-qr-label__field--model,
.asset-qr-label__field--location {
  color: var(--el-text-color-regular);
  font-size: 13px;
}

.asset-qr-label__field--serial {
  color: var(--el-text-color-secondary);
  font-size: 12px;
}
