...
import { CircleCheck, CircleClose, Delete, Download, Edit, Filter, Upload } from "@element-plus/icons-vue";
...
const showModelFilters = ref(false);
const modelFilterDraft = reactive({ manufacturer: "", deviceType: "", fieldset: "", active: "" });
...
function mapFieldset(item: Record<string, unknown>): SearchableSelectOption {
  ...
  return { value: String(value.id), label: value.name, secondary: value.description || "", disabled: value.is_active === false, data: value };
}
...
const selectedFilterManufacturerOption = computed<SearchableSelectOption | null>(() => {
  const item = manufacturers.value.find((entry) => String(entry.id) === modelFilterDraft.manufacturer);
  return item ? mapDictionary(item as unknown as Record<string, unknown>) : null;
});
const selectedFilterDeviceTypeOption = computed<SearchableSelectOption | null>(() => {
  const item = deviceTypes.value.find((entry) => String(entry.id) === modelFilterDraft.deviceType);
  return item ? mapDictionary(item as unknown as Record<string, unknown>) : null;
});
const selectedFilterFieldsetOption = computed<SearchableSelectOption | null>(() => {
  const item = fieldsets.value.find((entry) => String(entry.id) === modelFilterDraft.fieldset);
  return item ? mapFieldset(item as unknown as Record<string, unknown>) : null;
});
const activeModelFilterCount = computed(() =>
  Number(Boolean(filterManufacturer.value)) +
  Number(Boolean(filterDeviceType.value)) +
  Number(Boolean(filterFieldset.value)) +
  Number(Boolean(active.value)),
);
...
function openModelFilters() {
  modelFilterDraft.manufacturer = filterManufacturer.value;
  modelFilterDraft.deviceType = filterDeviceType.value;
  modelFilterDraft.fieldset = filterFieldset.value;
  modelFilterDraft.active = active.value;
  showModelFilters.value = true;
}

function clearModelFilters() {
  modelFilterDraft.manufacturer = "";
  modelFilterDraft.deviceType = "";
  modelFilterDraft.fieldset = "";
  modelFilterDraft.active = "";
}

function applyModelFilters() {
  filterManufacturer.value = modelFilterDraft.manufacturer;
  filterDeviceType.value = modelFilterDraft.deviceType;
  filterFieldset.value = modelFilterDraft.fieldset;
  active.value = modelFilterDraft.active;
  searchModels();
  showModelFilters.value = false;
}
...
<template>
  <PageContainer>
    <template #toolbar>
      <PageToolbar>
        ...
        <template #filters>
          <ToolbarIconButton :icon="Filter" :label="t('asset.moreFilters')" :badge="activeModelFilterCount" @click="openModelFilters" />
        </template>
        <template v-if="canManage" #actions>
          <ToolbarIconButton :icon="Upload" :label="t('common.import')" @click="openImport" />
          <ToolbarIconButton :icon="Download" :label="t('common.export')" :loading="exporting" :disabled="exporting" @click="exportModels" />
        </template>
        <template #primary>
  ...
  <el-drawer v-model="showModelFilters" :title="t('asset.moreFilters')" size="360px" append-to-body>
    <div class="filter-drawer-form">
      <el-form label-position="top">
        <el-form-item :label="t('assetModel.manufacturer')">
          <SearchableSelect
            v-model="modelFilterDraft.manufacturer"
            :request="c.request"
            endpoint="/manufacturers/"
            :map-option="mapDictionary"
            :base-query="{ is_active: 'all' }"
            :selected-option="selectedFilterManufacturerOption"
            :placeholder="t('assetModel.manufacturer')"
          />
        </el-form-item>
        <el-form-item :label="t('assetModel.category')">
          <SearchableSelect
            v-model="modelFilterDraft.deviceType"
            :request="c.request"
            endpoint="/device-types/"
            :map-option="mapDictionary"
            :base-query="{ is_active: 'all' }"
            :selected-option="selectedFilterDeviceTypeOption"
            :placeholder="t('assetModel.category')"
          />
        </el-form-item>
        <el-form-item :label="t('assetModel.fieldset')">
          <SearchableSelect
            v-model="modelFilterDraft.fieldset"
            :request="c.request"
            endpoint="/custom-fieldsets/"
            :map-option="mapFieldset"
            :base-query="{ is_active: 'all' }"
            :selected-option="selectedFilterFieldsetOption"
            :placeholder="t('assetModel.fieldset')"
          />
        </el-form-item>
        <el-form-item :label="t('common.status')">
          <el-select v-model="modelFilterDraft.active" clearable :placeholder="t('common.status')">
    ...
    <template #footer>
      <div class="filter-drawer__footer">
        <el-button @click="clearModelFilters">{{ t('common.clearFilters') }}</el-button>
        <el-button @click="showModelFilters = false">{{ t('common.cancel') }}</el-button>
        <el-button type="primary" @click="applyModelFilters">{{ t('asset.applyFilters') }}</el-button>
      </div>
    </template>
  </el-drawer>
  ...
  <FormDialogShell v-model="showModal" :title="editing ? t('assetModel.editTitle') : t('assetModel.createTitle')" :description="t('assetModel.description')" size="medium" :saving="saving" :show-close="!saving" :close-disabled="saving" :close-on-click-modal="!saving" :close-on-press-escape="!saving">
    ...
    <template #footer><el-button :disabled="saving" @click="showModal = false">{{ t('common.cancel') }}</el-button><el-button type="primary" :loading="saving" :disabled="saving || !canManage" @click="saveModel">{{ editing ? t('common.save') : t('common.create') }}</el-button></template>
  </FormDialogShell>
