...
如果虚机还原后准备重新部署，并且不复用旧业务数据库，按以下顺序执行：

1. 核对系统时间、DNS、路由和 80/443/8001 端口；快照还原后时间不同步会影响软件源和证书校验。
2. 准备一个新的空 `utf8mb4` MariaDB 数据库，或在向导中选择本机 MariaDB。
3. 从项目根目录执行 `sudo bash deploy/install.sh`；使用外部数据库时传入完整环境文件和 `--config`。
4. 安装器会在空数据库上执行当前发布包中的迁移，然后配置服务和 Nginx。
5. 创建管理员后执行部署验收：
...
如果需要保留旧业务数据，不要按全新安装处理，也不要清空数据库；应使用
`INSTALL_MODE=auto`/`upgrade`，保留原环境文件和已应用迁移文件，并先创建可恢复的数据库备份。
...
生产默认复用健康的 MariaDB 连接 60 秒，并启用连接健康检查，以减少高频请求反复建立连接的开销。`DB_CONN_MAX_AGE` 应不高于数据库或网络设备关闭空闲连接的时间；如现场的 `wait_timeout` 更短，请相应调低。安装器默认生成 3 个 Gunicorn worker，可在环境文件中设置 `GUNICORN_WORKERS`，再结合 CPU、内存和真实流量调整。
...
Infrix 内置备份功能仅支持生产环境 MariaDB。安装器默认将
`INFRIX_BACKUP_DIR` 设置为 `/var/lib/infrix/backups`；也可以在
`/etc/infrix/infrix.env` 中指定其他绝对路径。备份目录由 `infrix` 服务账号
拥有，权限为 `700`，必须位于应用、静态文件和媒体目录之外，并且不会由
Nginx 直接提供访问。

在“系统维护 → 备份与恢复”中，拥有现有 `system.reset` 权限的系统管理员可以
创建、下载、校验、恢复和删除本机备份。每个归档包含 `manifest.json`、压缩的
MariaDB 数据库导出，以及存在时的完整 `MEDIA_ROOT` 目录。备份可能包含敏感
业务数据，应按数据库和环境文件同等级别保护。

同一套服务也提供管理命令。执行前加载生产环境文件，但不要打印文件内容：
...
恢复只接受配置目录中已经存在的合法归档。系统会校验归档、迁移状态、数据库
导出和媒体路径，先创建 `pre-restore` 恢复前安全备份，不会自动执行迁移，成功
后会使当前登录会话失效。如果数据库或媒体恢复失败，接口和服务日志会分别给出
已完成阶段及安全备份文件名，便于管理员按实际状态处理。当前没有内置定时备份
和保留策略；如有需要，应由部署方另行评审并安排外部运维任务。
...
核对 `DB_HOST`、`DB_PORT`、`DB_NAME`、`DB_USER`、`DB_PASSWORD`、数据库授权和网络策略。全新数据库必须为空；升级时必须保留已应用的迁移文件，并有可读取的升级前备份。不要删除迁移记录或已应用的迁移文件来绕过错误。
