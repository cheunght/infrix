...
| Installer upgrade artifacts | `/var/backups/infrix` | Restricted temporary database and Nginx backups created by the installer. |
| Application backup archives | `/var/lib/infrix/backups/*.tar.gz` | `INFRIX_BACKUP_DIR`; owned by the service account, directory `700`, archives `600`. |
...
settings. It reuses healthy MariaDB connections for 60 seconds by default and
enables connection health checks.

Set the connection lifetime explicitly in the environment file when needed:
...
Keep `DB_CONN_MAX_AGE` no higher than the idle-connection timeout configured by
the database or network devices; lower it when that timeout is shorter.
...
### Process and database connection settings

Production reuses MariaDB connections for 60 seconds and enables connection
health checks by default. Keep `DB_CONN_MAX_AGE` no higher than the idle-
connection timeout configured by the database or network devices. The reference
Gunicorn setup uses 3 workers; change the systemd `--workers` value when the
host resources and business traffic require it, then verify the result.
...
stop and restore the corresponding source or migration path before changing the
database; do not delete `django_migrations` records to bypass the check.
...
When using the supplied systemd/Nginx layout, run the full read-only acceptance
check as well:

```bash
sudo env BASE_URL=https://<hostname> bash /opt/infrix/deploy/verify-release.sh
```

## 13. Backup, restore, and daily operations

The built-in backup service supports production MariaDB only. Set
`INFRIX_BACKUP_DIR` in `/etc/infrix/infrix.env`; the automatic installer uses
`/var/lib/infrix/backups` by default. Keep that directory outside the
application, static, and media directories. It must not be exposed by Nginx.

The archive contains `manifest.json`, `database.sql.gz`, and the complete
`MEDIA_ROOT` tree when it exists. The MariaDB `mariadb-dump` client is preferred;
`mysqldump` is accepted as a fallback. The service passes credentials through a
mode-`600` temporary defaults file and never puts the database password in a
command argument or log. Treat archives and the environment file, including
the Fernet key, as sensitive data.

The web page is **System Settings → Maintenance → Backup & Restore** and is
restricted to the existing `system.reset` capability. It accepts only archives
already present in the configured directory; arbitrary uploads are not
supported. Restore requires the exact confirmation `RESTORE INFRIX`, validates
the manifest, migration state, gzip dump, and archive paths, creates a
pre-restore safety backup first, and never runs migrations automatically. A
successful restore invalidates existing sessions. A partial failure reports the
database/media stage and safety-backup filename for deliberate recovery.

Run the read-only preflight or the same core service from a shell with the
...
Built-in scheduling and retention are not implemented. If the organization
needs a schedule, review an external systemd or operations schedule separately;
do not run overlapping backup and restore commands.
