...
| 安装器升级文件 | `/var/backups/infrix` | 安装器生成的受限临时数据库和 Nginx 备份。 |
| 应用备份归档 | `/var/lib/infrix/backups/*.tar.gz` | 由 `INFRIX_BACKUP_DIR` 指定；目录 `700`，归档 `600`，归服务账号所有。 |
...
生产 MariaDB 默认复用连接 60 秒，并启用连接健康检查。可在环境文件中显式设置：
...
`DB_CONN_MAX_AGE` 不应高于数据库或网络设备的空闲连接超时；现场超时更短时应相应调低。
...
“组织权限”页签按用户账号、人员管理、部门、角色、LDAP / AD 的顺序显示。当前账号能看到哪些页签，由权限决定。

人员记录可以没有登录账号。姓名必填，员工编号、所属部门、单位、联系方式和启用状态可选。创建本地账号时，管理员可以关联一个尚未绑定账号的人员；如果不选择，创建账号时系统会自动建立人员记录。

资产可以没有使用人，也只能有一个当前使用人。新建或编辑资产时可以选择人员，也可以在资产详情中执行指定、转交和归还。只有启用人员可以被指定。转交会写入使用历史，资产状态保持不变。人员所属部门只是人员资料，不是资产归属字段。

资产台账支持管理员批量指定或批量转交当前页选中的资产，每批最多 100 项。每项都会返回处理结果；成功项取消选中，失败项保留选中，修正后可以重试。

资产导入中的 `assigned_person_employee_no`、`assigned_person_name`、`assigned_person_department` 和 `assignment_reason` 用于匹配已有人员，导入不会自动创建人员。资产导出会包含使用人姓名、员工编号、部门、单位和联系方式。
...
生产 MariaDB 默认复用连接 60 秒并启用连接健康检查；`DB_CONN_MAX_AGE` 不应高于
数据库或网络设备的空闲连接超时。参考 Gunicorn 配置使用 3 个 worker；需要调整时
修改 systemd 的 `--workers` 参数，并根据主机资源和业务流量验证。
...
保留所有已经应用的迁移文件；如果数据库有迁移记录但源码缺少对应文件，应先恢复
包含该迁移的源码或迁移路径，再修改数据库，不要删除 `django_migrations` 记录绕过检查。
...
使用随附的 systemd/Nginx 布局时，还可以运行完整的只读验收：
...
内置备份服务仅支持生产环境 MariaDB。在 `/etc/infrix/infrix.env` 中设置
`INFRIX_BACKUP_DIR`；自动安装器默认使用 `/var/lib/infrix/backups`。该目录
必须位于应用、静态文件和媒体目录之外，不能通过 Nginx 暴露。

归档包含 `manifest.json`、`database.sql.gz`，以及存在时的完整 `MEDIA_ROOT`
目录。服务优先使用 MariaDB 的 `mariadb-dump`，找不到时兼容 `mysqldump`。
数据库凭据通过权限为 `600` 的临时 defaults 文件传递，不会放入命令参数或日志。
备份归档和包含 Fernet 密钥的环境文件都属于敏感数据，必须按同等级别保护。

页面入口是“系统设置 → 系统维护 → 备份与恢复”，仅现有 `system.reset` 权限
可用。系统只接受配置目录中已经存在的归档，不支持任意文件上传。恢复必须输入
准确的 `RESTORE INFRIX`，并校验 manifest、迁移状态、gzip 数据库导出和归档路径；
恢复前会先创建 `pre-restore` 安全备份，不会自动执行迁移。恢复成功后会使已有登录
会话失效；发生部分失败时会返回数据库/媒体阶段和安全备份文件名，便于人工处理。

在已加载生产环境文件的 shell 中执行只读检查或同一套核心服务：
...
当前没有内置定时备份和保留策略。如需定时执行，应由部署方另行评审外部 systemd
或运维计划，并确保备份与恢复不会并发运行。
...
新安装确认数据库为空；升级确认备份存在，且源码包含所有已记录迁移。确认应用账号
可以创建和修改表及索引，再执行：
