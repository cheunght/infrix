...
After restoring a VM for redeployment, prepare a new empty database and use
the fresh-install path. If old data must be retained, use `auto`/`upgrade`,
preserve the existing environment and applied migration files, and back up the
database first.
...
## Backup and restore after installation

The installer creates the application backup directory at
`/var/lib/infrix/backups` by default and writes the path to
`INFRIX_BACKUP_DIR` in `/etc/infrix/infrix.env`. This is separate from
`/var/backups/infrix`, which the installer uses for upgrade-time database and
Nginx configuration artifacts. Built-in Backup & Restore supports production
MariaDB only and is described in the [automatic installation guide](INSTALL_VM.md)
and [manual deployment guide](MANUAL_DEPLOYMENT.md).

The installer does not schedule backups or apply retention. Use the built-in
page or the shared `check_backup`, `create_backup`, and `restore_backup`
management commands after loading the service environment. Restore requires
`RESTORE INFRIX`, creates a safety backup first, and never runs migrations
automatically.

Production reuses healthy MariaDB connections for 60 seconds with connection
health checks enabled. The generated Gunicorn service defaults to 3 workers.
Set `DB_CONN_MAX_AGE` or `GUNICORN_WORKERS` in the environment file when
needed, then validate the values against the database idle timeout and observed
...
When using the supplied systemd/Nginx layout, also run the full acceptance
check:
