...
虚机还原后重新部署时，请准备新的空数据库并按全新安装执行。需要保留旧数据时必须走 `auto`/`upgrade`，保留原环境文件和已应用迁移文件，并先完成备份。
...
安装器默认创建应用备份目录 `/var/lib/infrix/backups`，并将路径写入
`/etc/infrix/infrix.env` 的 `INFRIX_BACKUP_DIR`。该目录不同于
`/var/backups/infrix`；后者用于安装器升级期间的数据库和 Nginx 配置文件。
内置备份与恢复仅支持生产环境 MariaDB，具体操作见[自动安装文档](DEPLOYMENT.zh-CN.md)
和[中文手工部署文档](MANUAL_DEPLOYMENT.zh-CN.md)。

安装器不会自动创建定时备份，也不会执行保留清理。安装完成后可在页面中操作，或
加载服务环境后使用共用的 `check_backup`、`create_backup` 和 `restore_backup`
管理命令。恢复必须输入 `RESTORE INFRIX`，开始前会先创建安全备份，并且不会自动
执行数据库迁移。

生产 MariaDB 默认使用 60 秒连接复用并启用连接健康检查；安装器生成的 Gunicorn
默认使用 3 个 worker。需要调整时，在环境文件中设置 `DB_CONN_MAX_AGE` 或
`GUNICORN_WORKERS`，并根据数据库空闲连接超时、CPU、内存和真实流量验证结果。
...
使用随附 systemd/Nginx 布局时，再执行完整验收：
