...
### Fresh deployment after restoring a VM

If a restored VM is being redeployed without reusing its old business database:

1. Check the system clock, DNS, routes, and ports 80, 443, and 8001. Snapshot
   restores can leave the clock unsynchronized, which affects package and
   certificate validation.
2. Prepare a new empty `utf8mb4` MariaDB database, or choose local MariaDB in
   the wizard.
3. Run `sudo bash deploy/install.sh` from the project root. For an external
   database, provide the completed environment file with `--config`.
4. The installer applies the migrations in the release package to the empty
   database, then configures the service and Nginx.
5. Create the administrator and run the acceptance check:

   ```bash
   sudo env BASE_URL=https://<hostname> bash deploy/verify-release.sh
   ```

If old business data must be retained, do not treat the deployment as fresh or
empty the database. Use `INSTALL_MODE=auto`/`upgrade`, preserve the existing
environment file and applied migration files, and create a restorable database
backup first.
...
Production reuses healthy MariaDB connections for 60 seconds and enables
connection health checks, reducing the cost of reconnecting on busy request
paths. Keep `DB_CONN_MAX_AGE` no higher than the idle-connection timeout used by
the database or network devices; lower it when that timeout is shorter. The
installer defaults to 3 Gunicorn workers. Set `GUNICORN_WORKERS` in the
environment file and tune it using observed CPU, memory, and traffic.
...
Infrix's built-in backup feature supports production MariaDB only. The
installer sets `INFRIX_BACKUP_DIR` to `/var/lib/infrix/backups` unless another
absolute path is supplied in `/etc/infrix/infrix.env`. The directory is owned by
the `infrix` service account with mode `700`; keep it outside the application,
static, and media directories. It is never served by Nginx.

From **System Maintenance → Backup & Restore**, a system administrator with
the existing `system.reset` capability can create, download, validate, restore,
or delete local archives. Each archive contains `manifest.json`, a compressed
MariaDB dump, and the complete `MEDIA_ROOT` tree when that directory exists.
Archives may contain sensitive business data; protect them like the database
and environment file.

The same service is available from the management commands. Run these commands
with the production environment loaded, without printing the file:

```bash
sudo -u infrix bash -c 'set -a; . /etc/infrix/infrix.env; set +a; cd /opt/infrix/backend; exec .venv/bin/python manage.py check_backup --json'
sudo -u infrix bash -c 'set -a; . /etc/infrix/infrix.env; set +a; cd /opt/infrix/backend; exec .venv/bin/python manage.py create_backup'
sudo -u infrix bash -c 'set -a; . /etc/infrix/infrix.env; set +a; cd /opt/infrix/backend; exec .venv/bin/python manage.py restore_backup infrix-backup-YYYYMMDD-HHMMSS.tar.gz --confirm-restore "RESTORE INFRIX"'
```

Restore accepts only an existing archive in the configured directory. It
validates the archive, migration state, database dump, and media paths; creates
a `pre-restore` safety backup first; never runs migrations automatically; and
invalidates current sessions after success. If database or media recovery fails,
the response and server log identify the completed stage and safety-backup
filename so an administrator can recover deliberately. Built-in scheduling and
retention are not implemented; use an externally reviewed operational schedule
only if the site requires one.

Check the deployment with:

```bash
sudo systemctl is-active infrix nginx
# Check this as well when MariaDB is local:
...
network policy. A fresh database must be empty. An upgrade must preserve every
