...
- Production MariaDB backup and restore, protected configuration keys, and
  deployment diagnostics.

## Post-deployment verification

When using the supplied installer, run the read-only acceptance check:

```bash
sudo env BASE_URL=https://<hostname> bash deploy/verify-release.sh
```

It checks production security settings, Django checks and migrations, core
tables, rack U-position integrity, static assets, API responses, frontend
routes, and the Nginx entry point. Run the entry-point diagnosis separately
when needed:
...
- [Local MariaDB backup and restore](docs/INSTALL_VM.md#local-backup--restore)
