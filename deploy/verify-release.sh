...
echo "环境文件：$ENV_FILE"
echo "检查地址：${BASE_URL%/}"
application_backup_dir="${INFRIX_BACKUP_DIR:-}"
if [[ -z "$application_backup_dir" ]]; then
  application_backup_dir="$(env_value INFRIX_BACKUP_DIR)"
fi
application_backup_dir="${application_backup_dir:-/var/lib/infrix/backups}"
echo "最近备份：$application_backup_dir"
find "$application_backup_dir" -maxdepth 1 -type f -printf '%TY-%Tm-%Td %TH:%TM %p\n' 2>/dev/null | sort -r | head -5 || true
