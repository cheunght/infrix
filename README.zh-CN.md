...
- 资产台账、标准资产型号、可复用字段集、自定义字段库、导入导出、标签、生命周期数据和可选使用人。
- 数据中心、机房、机柜、U 位分配和容量视图。
- 故障登记、维修流程、维修成本和维修历史。
- 软件许可、盘点任务、备件、库存和库存流水。
- 用户账号、人员、部门、角色、LDAP/Active Directory 集成和审计日志。
- 语言、安全、SMTP、通知、品牌和系统维护设置。
- 生产环境 MariaDB 备份与恢复、受保护配置密钥和部署诊断。

## 部署后验收

使用随附安装器部署后，可执行完整的只读验收：

```bash
sudo env BASE_URL=https://<hostname> bash deploy/verify-release.sh
```

该检查会验证生产安全配置、Django 检查与迁移、核心数据表、机柜 U 位完整性、静态资源、API、前端路由和 Nginx 入口。入口诊断也可以单独执行：
...
- [本机 MariaDB 备份与恢复](docs/DEPLOYMENT.zh-CN.md#本机备份与恢复)
